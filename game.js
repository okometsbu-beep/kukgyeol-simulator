(function () {
  ...
  const SHARED_PERSONAL_MONTHLY = 850000;
  ...
  const SCENE_LIBRARY = [
    ...
    { id: "shadow_mirror", title: "서로의 그림자", objective: "끌림 속에 숨은 욕구와 상대의 선택권을 함께 본다", bg: "first-night" },
    ...
    { id: "housing_choice", title: "어디에서 어떻게 살까", objective: "첫 거주지 합의에 맞춰 월 부담과 함께 사는 시간을 계산한다", bg: "airport" },
  ...
  const MARRIAGE_MONTH = {
    wedding_place: 0, wedding_budget: 0, wedding_day: 1, housing_choice: 1, new_home: 2, household_money: 3,
    partner_work: 4, family_remittance: 5, intimacy_distance: 6, intimacy_talk: 6,
    child_plan: 7, health_check: 8, pregnancy_try: 9, pregnancy_result: 10,
    pregnancy_months: 13, birth: 19, newborn_night: 19, childcare_plan: 20,
    marriage_crisis: 21, hidden_after_marriage: 21, anniversary_inquiry: 23, family_decision: 24
  };
  ...
  const HOUSING_POLICY = {
    ...
    ltv: .7,
    dsr: .4,
    mortgageCap: 600000000,
    policyHomeCap: 600000000,
    policyLoanCap: 320000000,
    policyRate: .034,
    bankRate: .046,
    termYears: 30
  };
  ...
  const HOUSING_OPTIONS = [
    { id: "rent_short_stay", tenure: "rent", region: "korea", name: "서울 단기 거주 원룸", area: 23, bedrooms: 1, deposit: 0, monthly: 1650000, fit: -4, note: "보증금 없이 바로 들어가지만 비싸고 오래 살 집은 아니다" },
    { id: "rent_villa_small", tenure: "rent", name: "서울 외곽 빌라 14평 월세", area: 46, bedrooms: 1, deposit: 20000000, monthly: 950000, fit: -2, note: "둘이 시작하기엔 가능하지만 아이 계획에는 빠르게 좁아진다" },
    { id: "rent_villa_family", tenure: "rent", name: "서울 역세권 빌라 20평 월세", area: 66, bedrooms: 2, deposit: 50000000, monthly: 1450000, fit: 1, note: "출퇴근과 방 두 개를 확보하는 현실적인 타협" },
    { id: "rent_apartment", tenure: "rent", name: "서울 외곽 구축 아파트 24평 월세", area: 79, bedrooms: 3, deposit: 80000000, monthly: 2200000, fit: 3, note: "생활 만족은 높지만 월세가 가계의 목을 매달 잡는다" },
    { id: "buy_villa", tenure: "buy", name: "서울 외곽 빌라 18평 매수", area: 59, bedrooms: 2, price: 340000000, fit: 1, note: "게임상 추정 시세. 환금성과 수리비 위험을 감수하고 내 집을 만든다" },
    { id: "buy_apartment_small", tenure: "buy", name: "서울 외곽 구축 아파트 18평 매수", area: 59, bedrooms: 2, price: 850000000, fit: 2, note: "서울 평균보다 낮은 외곽 구축 가정. 현금과 DSR 문턱이 매우 높다" },
    { id: "buy_apartment_family", tenure: "buy", name: "서울 25평대 아파트 매수", area: 84, bedrooms: 3, price: 1500000000, fit: 4, note: "2026년 서울 평균권을 반영한 게임상 추정치. 대출보다 자기자본이 먼저 필요하다" }
  ];

  const LOCAL_HOUSING_INDEX = { vn: .42, cn: .62, th: .52, jp: .95, ph: .43, kh: .32 };
  const DISTANCE_HOUSING_OPTIONS = [
    { id: "distance_monthly", tenure: "rent", region: "distance", name: "각자 집 유지 · 월 1회 왕래", area: 0, bedrooms: 0, deposit: 0, monthly: 1800000, fit: -2, note: "일은 지키지만 함께 사는 시간이 적고 항공료 변동을 감수한다" },
    { id: "distance_halfmonth", tenure: "rent", region: "distance", name: "한 달씩 번갈아 거주", area: 0, bedrooms: 1, deposit: 5000000, monthly: 2750000, fit: 1, note: "체류 공간 두 곳과 이동비가 필요하지만 생활을 실제로 비교할 수 있다" },
    { id: "distance_base", tenure: "rent", region: "distance", name: "현지 거점집 + 한국 출장방", area: 46, bedrooms: 2, deposit: 15000000, monthly: 3400000, fit: 2, note: "같이 지내는 시간은 늘지만 두 나라 고정비를 동시에 낸다" }
  ];

  function localHousingOptions() {
    const country = getCountry();
    const index = LOCAL_HOUSING_INDEX[country?.id] || .55;
    const scaled = amount => Math.round(amount * index / 10000) * 10000;
    return [
      { id: `local_short_${country.id}`, tenure: "rent", region: "local", name: `${country.name} 가구 포함 단기 스튜디오`, area: 28, bedrooms: 1, deposit: 0, monthly: scaled(1500000), fit: -2, note: "바로 시작할 수 있지만 여행자 요금이라 장기 생활에는 비싸다" },
      { id: `local_home_${country.id}`, tenure: "rent", region: "local", name: `${country.name} 생활권 1베드룸`, area: 45, bedrooms: 1, deposit: scaled(18000000), monthly: scaled(1100000), fit: 1, note: "그녀의 일과 가족 생활권에서 1년을 시험하는 현실적인 선택" },
      { id: `local_family_${country.id}`, tenure: "rent", region: "local", name: `${country.name} 가족형 2베드룸`, area: 68, bedrooms: 2, deposit: scaled(30000000), monthly: scaled(1650000), fit: 3, note: "공간은 넉넉하지만 줄어든 내 현지 수입에 부담이 커진다" }
    ];
  }

  function housingOptions() {
    if (state.homeCountry === "local") return localHousingOptions();
    if (state.homeCountry === "distance") return DISTANCE_HOUSING_OPTIONS;
    return HOUSING_OPTIONS;
  }

  function housingFallback() {
    if (state.homeCountry === "local") return { id: `local_fallback_${getCountry().id}`, tenure: "rent", region: "local", name: `${getCountry().name} 한 달 임시 숙소`, area: 20, bedrooms: 1, deposit: 0, monthly: Math.round(1900000 * (LOCAL_HOUSING_INDEX[getCountry().id] || .55)), fit: -5 };
    if (state.homeCountry === "distance") return { id: "distance_fallback", tenure: "rent", region: "distance", name: "각자 기존 집 · 월 1회 만남", area: 0, bedrooms: 0, deposit: 0, monthly: 2000000, fit: -4 };
    return { id: "korea_fallback", tenure: "rent", region: "korea", name: "서울 한 달 임시 숙소", area: 18, bedrooms: 1, deposit: 0, monthly: 1950000, fit: -5 };
  }

  function housingOptionById(id) {
  ...
  const REMATCH_ENDINGS = new Set(["breakup", "deadline", "marriage_rejected", "false_accusation", "false_broker_accusation", "unproven_accusation", "safe_exit"]);
  ...
  function tripQuote(kind = "standard") {
    const travel = getCountry()?.travel || { airfare: 450000, lodgingNight: 80000, date: 130000, luxury: 700000 };
    const stay = travel.lodgingNight * 4;
    if (kind === "luxury") return travel.airfare + stay + travel.luxury;
    if (kind === "interpreter") return travel.airfare + stay + travel.date + 300000;
    return travel.airfare + stay + travel.date;
  }
  ...
  function lifePhotoStyle(art, mode = "real") {
    const cols = art.cols || 4;
    const rows = art.rows || 4;
    const cell = mode === "profile" ? art.profileCell
      : mode === "settled" ? (art.settledCell ?? art.realCell)
        : mode === "newlywed" ? (art.newlywedCell ?? art.profileCell)
          : (art.realCell ?? art.profileCell);
    const col = cell % cols;
    const row = Math.floor(cell / cols);
    const x = cols === 1 ? 0 : (col / (cols - 1)) * 100;
    const y = rows === 1 ? 0 : (row / (rows - 1)) * 100;
  ...
  function routeSetupHeader() {
    ...
    const country = getCountry();
    const playerCash = rematchCarry?.cash ?? DATA.players.find(x => x.id === setup.playerId)?.cash ?? 0;
    const selectedPlan = (DATA.paymentPlans[setup.routeId] || []).find(item => item.id === setup.planId);
    const cash = formatWon(Math.max(0, playerCash - (rematchCarry ? 0 : selectedPlan?.price || 0)));
    const copy = {
    ...
    return `<div class="route-setup-bar route-${route}${rematchCarry ? " is-rematch" : ""}"><div><span class="route-setup-logo">${copy[0]}</span><strong>${copy[1]}</strong><small>${rematchCarry ? `이전 관계 손실 승계 · ${rematchCarry.attempt}회차` : copy[2]}</small></div><div class="dating-wallet">${rematchCarry ? "남은 자금 " : selectedPlan ? "결제 뒤 " : "내 자금 "}${cash}</div></div>`;
  }
  ...
  function renderSetup() {
    ...
    items.forEach(item => {
      ...
      if (setup.step === 0) {
        const visibleNeeds = (item.needs || []).slice(0, 2);
        const needPreview = `${visibleNeeds.map(need => `<span>${escapeHtml(need.label)}</span>`).join("")}${(item.needs || []).length > 2 ? `<span>외 ${(item.needs || []).length - 2}개 · 발견 뒤 확인</span>` : ""}`;
        const compulsion = item.compulsion || {};
  ...
  function openPlayerPreview(id) {
    const player = DATA.players.find(item => item.id === id);
    if (!player) return;
    const compulsion = player.compulsion || {};
    const stats = [["외모", player.appearance], ["말재주", player.charm], ["공감", player.empathy], ["판단", player.reason], ["배짱", player.courage]];
    openModal("주인공 상세", `${player.name} · ${player.age}세`, `<div class="player-preview"><div class="player-profile-head"><div class="player-profile-photo" style="${photoStyle(player.art, "man")}"></div><div><h3>${escapeHtml(player.job)}</h3><p>${escapeHtml(player.hook)}</p><small>약점 · ${escapeHtml(player.flaw)}</small></div></div><div class="profile-finance"><span>현금 <b>${formatWon(player.cash)}</b></span><span>주거 종잣돈 <b>${formatWon(player.housingAsset || 0)}</b></span><span>월수입 <b>${formatWon(player.income)}</b></span><span>월 고정비 <b>${formatWon(player.monthlyCommitment + player.singleLiving)}</b></span></div><div class="profile-stat-list">${stats.map(([label, value]) => `<div><span>${label}</span><div class="profile-stat-track"><i style="width:${value * 10}%"></i></div><b>${value}</b><small>/10</small></div>`).join("")}</div><article class="compulsion-card"><span>강박 · 유혹 버튼</span><h3>${escapeHtml(compulsion.label || "이름 붙이지 못한 강박")}</h3><p>${escapeHtml(compulsion.truth || "")}</p><dl><div><dt>즉시 보상</dt><dd>${escapeHtml(compulsion.reward || "")}</dd></div><div><dt>후유증</dt><dd>${escapeHtml(compulsion.aftertaste || "")}</dd></div></dl></article><div class="confirm-actions"><button id="cancel-player-preview" class="ghost-button" type="button">다른 인물 보기</button><button id="confirm-player-preview" class="primary-button" type="button">이 인물로 시작</button></div></div>`, element => {
      element.querySelector("#cancel-player-preview").addEventListener("click", closeModal);
      element.querySelector("#confirm-player-preview").addEventListener("click", () => {
        setup.playerId = id;
        closeModal();
        renderSetup();
      });
  ...
  function chooseSetupItem(id) {
    if (setup.step === 0) return openPlayerPreview(id);
  ...
  function estimatePartnerIncome(countryId, job) {
    const countryBase = { vn: 950000, cn: 1850000, th: 1250000, jp: 3050000, ph: 1050000, kh: 720000 }[countryId] || 1400000;
    let multiplier = 1;
    if (/운영|은행|간호사|무역|회계 담당|관리자|팀장/.test(job)) multiplier = 1.35;
    else if (/프리랜서|번역|한국어 강사|여행사|호텔 프런트|온라인 판매/.test(job)) multiplier = 1.16;
    else if (/대학원생|보조|카페|의류점|미용사|식당/.test(job)) multiplier = .82;
    const volatility = .86 + Math.random() * .3;
    return Math.max(450000, Math.round(countryBase * multiplier * volatility / 50000) * 50000);
  }
  ...
  function buildCandidates() {
    ...
    return faceSlots.map((faceSlot, index) => {
      ...
      const estimatedIncome = estimatePartnerIncome(setup.countryId, selectedJob);
      const jobPortability = /온라인|번역|한국어|무역|여행/.test(selectedJob) ? 7 : /대학원|은행|간호|가게|운영/.test(selectedJob) ? 3 : 5;
      const shadowProfile = (behavior.shadowIds || []).map((id, shadowIndex) => ({ id, intensity: clamp(2 + ((faceSlot + index + shadowIndex) % 3), 2, 4) }));
      return {
        ...base,
        ...
        mbti: behavior.mbti || "ISFP",
        hiddenReaction: behavior.hiddenReaction || "cautious",
        hiddenNeed: behavior.hiddenNeed || "말보다 반복되는 행동을 본다",
        ...
        privateTraits: {
          ...
          tattoo,
          health,
          attractiveness: 4 + Math.floor(Math.random() * 7),
          desiredChildren: pick([0, 1, 1, 2, 2, 2, 3, 3, 4]),
          chemistry: 35 + Math.floor(Math.random() * 61),
          pace: pick(["slow", "slow", "warm", "fast"]),
          shadowDesires: shadowProfile,
          secrets: secretPool
        },
  ...
  function chooseChildIntent(partner) {
    const desired = partner.privateTraits?.desiredChildren;
    if (Number.isFinite(desired)) {
      if (desired === 0) return "no";
      if (desired === 1) return Math.random() < .5 ? "later" : "yes";
      if (desired >= 2) return Math.random() < .82 ? "yes" : "later";
    }
  ...
  function revealProfile(keys, source, result = null) {
    ...
    if (newlyKnown.length) state.profileUnread = (state.profileUnread || 0) + newlyKnown.length;
  ...
  const PROFILE_LABELS = {
    ...
    personalityCore: "반복해서 드러난 성격", mbti: "MBTI", speechTone: "말투와 표현 습관", motive: "국제연애를 시작한 이유", intimacyBoundary: "친밀감의 경계",
    childIntent: "아이에 대한 실제 생각", desiredChildren: "원하는 자녀 수", photoReality: "사진과 실제 모습의 차이", attraction: "내가 느낀 외적 매력", height: "키", bodyType: "체형",
  ...
  function awardScore(amount, label, result = null, type = "play") {
    if (!amount) return;
    state.score = Math.max(0, (state.score || 0) + amount);
    state.scoreLog = [{ amount, label, type, day: state.elapsedDays }, ...(state.scoreLog || [])].slice(0, 40);
    if (result) result.scoreEvents = [...(result.scoreEvents || []), { amount, label, type }];
  }

  function needKnowledgeKey(need) {
    return {
      behavior: "personalityCore",
  ...
  function needSatisfied(need) {
    ...
    const knowledgeKey = needKnowledgeKey(need);
    if (knowledgeKey && !state.knownProfile?.[knowledgeKey]) return false;
    if (need.kind === "behavior") return (need.values || []).includes(partner.behavior?.id);
    if (need.kind === "attractiveness") return (traits.attractiveness || 0) >= (need.min || 0);
    if (need.kind === "cup") {
      const cup = String(traits.braSize || "").replace(/[^A-Z]/g, "").charAt(0);
      return "ABCDEFG".indexOf(cup) >= "ABCDEFG".indexOf(need.min || "A");
    }
    if (need.kind === "body") return (need.values || []).includes(traits.bodyType);
    if (need.kind === "ageGap") return ageGap() >= (need.min || 0);
    if (need.kind === "children") return (traits.desiredChildren || 0) >= (need.min || 0);
    if (need.kind === "chemistry") return (traits.chemistry || 0) >= (need.min || 0);
    if (need.kind === "tattoo") return traits.tattoo === need.value;
    if (need.kind === "career") return !/대학원생/.test(partner.job || "") && (partner.jobPortability || 0) >= 4;
    if (need.kind === "language") return (partner.languageLevel || 0) >= (need.min || 0);
    if (need.kind === "health") return !/수술|통증/.test(traits.health || "");
    return false;
  }

  function needScoreValue(need) {
    const generatedSpecKinds = new Set(["attractiveness", "cup", "body", "ageGap", "children", "chemistry", "tattoo", "health"]);
    return generatedSpecKinds.has(need.kind) ? Math.round(need.points * .65) : need.points;
  }

  function evaluateNeeds(result = null) {
    const needs = getPlayer()?.needs || [];
    const fulfilled = state.fulfilledNeeds || (state.fulfilledNeeds = []);
    needs.forEach(need => {
      if (fulfilled.includes(need.id) || !needSatisfied(need)) return;
      fulfilled.push(need.id);
      const amount = needScoreValue(need);
      awardScore(amount, `니즈 달성 · ${need.label}`, result, "need");
    });
  }

  function shadowData(id) {
    return DATA.shadowDesires?.[id] || { id, label: id, icon: "◼", impulse: "아직 설명되지 않은 욕구", regulated: "서로의 선택권을 지킨다.", distorted: "상대의 선택권을 지운다.", compatible: [] };
  }

  function playerShadowTags(id) {
    const tags = [];
    const rules = [
      ["control", /shadow_lead|cross_override|work_forbid|child_demand|intimacy_pressure|money_partner_only/],
      ["surrender", /shadow_follow|invite_follow_her|cross_accept_terms|money_all_partner|intimacy_consent/],
      ["possession", /shadow_test|snoop|check_phone|secret_trace|fight_win|distance_check_phone|accuse/],
      ["rescue", /pay_crisis|gift|support|hire_help|remit_generous|wedding_pay_extra|rescue/],
      ["escape", /rush_answer|sign_fast|marry_fast|move_local|move_korea|cross_overpromise/],
      ["status", /why_status|luxury|surprise_upgrade|wedding_large|wedding_both|brag/],
      ["security", /budget|contract|money_three|official_docs|housing_|work_training|shared_plan|verify_transfer/],
      ["adoration", /flirt|confess|reassure|date_rest|couple_reconnect|pickup_line|daily_play/],
      ["certainty", /investigat|field_|digital_|trace|official_docs|final_source|check_phone|fight_win/],
      ["dependence", /work_forbid|money_all_partner|cross_override|intimacy_pressure/]
    ];
    rules.forEach(([tag, pattern]) => { if (pattern.test(id)) tags.push(tag); });
    if (/shadow_name_it|shadow_pact|mutual_boundaries|fight_apologize|fight_pause/.test(id)) {
      (state.playerShadow || []).forEach(item => tags.push(item.id));
    }
    return [...new Set(tags)];
  }

  function shadowChoiceMode(id) {
    if (/shadow_name_it|shadow_pact|mutual_boundaries|cross_negotiate|money_three_accounts|intimacy_consent|fight_apologize|fight_pause/.test(id)) return "regulated";
    if (/shadow_test|cross_override|work_forbid|snoop|check_phone|secret_trace|fight_win|intimacy_pressure|child_demand|rush_answer/.test(id)) return "distorted";
    if (id === "shadow_bury") return "denied";
    return "expressed";
  }

  function revealPartnerShadow(preferredId, source, result) {
    const shadows = getPartner().privateTraits?.shadowDesires || [];
    const known = state.knownPartnerShadows || (state.knownPartnerShadows = []);
    const next = shadows.find(item => item.id === preferredId && !known.includes(item.id)) || shadows.find(item => !known.includes(item.id));
    if (!next) return null;
    known.push(next.id);
    result.shadowUnlocked = [...(result.shadowUnlocked || []), next.id];
    state.shadowHistory.unshift({ type: "reveal", id: next.id, source, day: state.elapsedDays });
    state.shadowHistory = state.shadowHistory.slice(0, 24);
    return next.id;
  }

  function evaluatePartnerNeeds(id, result, checkSuccess) {
    if (!checkSuccess || !result) return;
    const behavior = getPartner().behavior;
    const category = behaviorCategory(id);
    if (!behavior || !category || !behavior.likes?.includes(category)) return;
    const observations = state.partnerNeedObservations || (state.partnerNeedObservations = {});
    observations[category] = (observations[category] || 0) + 1;
    const total = Object.values(observations).reduce((sum, value) => sum + value, 0);
    const fulfilled = state.fulfilledPartnerNeeds || (state.fulfilledPartnerNeeds = []);
    const needs = behavior.relationshipNeeds || [];
    [2, 4].forEach((threshold, index) => {
      const needId = `${behavior.id}_${index}`;
      if (total < threshold || fulfilled.includes(needId) || !needs[index]) return;
      fulfilled.push(needId);
      const amount = index === 0 ? 260 : 420;
      state.partnerMatchScore = (state.partnerMatchScore || 0) + amount;
      result.partnerNeedEvents = [...(result.partnerNeedEvents || []), { label: needs[index], amount }];
      awardScore(amount, `그녀의 니즈 충족 · ${needs[index]}`, result, "partner-need");
    });
  }

  function applyShadowDynamics(id, result, checkSuccess) {
    if (!result) return;
    const tags = playerShadowTags(id);
    const mode = shadowChoiceMode(id);
    const playerShadow = (state.playerShadow || []).find(item => tags.includes(item.id));
    const partnerShadows = getPartner().privateTraits?.shadowDesires || [];
    const partnerShadow = partnerShadows.find(item => tags.includes(item.id))
      || partnerShadows.find(item => playerShadow && shadowData(playerShadow.id).compatible?.includes(item.id));

    if (playerShadow) {
      playerShadow.expressed += 1;
      if (mode === "regulated" && checkSuccess) playerShadow.regulated += 1;
      if (mode === "distorted") playerShadow.distorted += 1;
    }

    state.shadowSignals = state.shadowSignals || {};
    tags.forEach(tag => { state.shadowSignals[tag] = (state.shadowSignals[tag] || 0) + 1; });
    if (checkSuccess && id === "shadow_name_it") revealPartnerShadow(partnerShadow?.id, "서로 욕구를 이름 붙인 대화", result);
    else if (checkSuccess && id === "shadow_pact") revealPartnerShadow(partnerShadow?.id, "주도권과 멈춤 규칙을 합의한 경험", result);
    else if (partnerShadow && (state.shadowSignals[partnerShadow.id] || 0) >= 2) revealPartnerShadow(partnerShadow.id, "반복되는 선택과 반응", result);

    if (mode === "regulated" && checkSuccess && playerShadow && partnerShadow) {
      const compatible = playerShadow.id === partnerShadow.id
        || shadowData(playerShadow.id).compatible?.includes(partnerShadow.id)
        || shadowData(partnerShadow.id).compatible?.includes(playerShadow.id);
      if (compatible) {
        state.shadowResonance = (state.shadowResonance || 0) + 1;
        state.shadowRisk = clamp((state.shadowRisk || 0) - 4, 0, 100);
        result.trust = (result.trust || 0) + 3;
        result.affection = (result.affection || 0) + 4;
        const amount = 320 + Math.min(480, state.shadowResonance * 40);
        result.shadowEvents = [...(result.shadowEvents || []), { type: "resonance", player: playerShadow.id, partner: partnerShadow.id, amount }];
        awardScore(amount, `그림자 공명 · ${shadowData(playerShadow.id).label} × ${shadowData(partnerShadow.id).label}`, result, "shadow");
        state.shadowHistory.unshift({ type: "resonance", player: playerShadow.id, partner: partnerShadow.id, day: state.elapsedDays });
      }
    }

    if (mode === "distorted") {
      const risk = playerShadow ? 12 + playerShadow.intensity * 2 : 10;
      state.shadowRisk = clamp((state.shadowRisk || 0) + risk, 0, 100);
      result.trust = (result.trust || 0) - 4;
      result.conflict = (result.conflict || 0) + 5;
      result.partnerFatigue = (result.partnerFatigue || 0) + 4;
      result.shadowEvents = [...(result.shadowEvents || []), { type: "distortion", player: playerShadow?.id || tags[0] || "unknown", risk }];
      state.shadowHistory.unshift({ type: "distortion", id: playerShadow?.id || tags[0] || "unknown", day: state.elapsedDays });
      if (state.shadowRisk >= 60) state.flags.shadowSpiral = true;
    } else if (mode === "denied") {
      state.shadowRisk = clamp((state.shadowRisk || 0) + 6, 0, 100);
      result.shadowEvents = [...(result.shadowEvents || []), { type: "denied", risk: 6 }];
    }
    state.shadowHistory = state.shadowHistory.slice(0, 24);
  }
  ...
  function unlockProfileFromChoice(id, result, checkSuccess) {
    ...
    if (id.startsWith("arrival_")) revealProfile(["height", "bodyType", "photoReality", "attraction"], "현지에서 직접 만남", result);
    if (id === "official_docs" && checkSuccess) revealProfile(["verifiedAge", "verifiedCity", "verifiedJob", "identityVerified"], "공식 서류와 원본 대조", result);
    if (id === "mutual_boundaries") successOnly(["childIntent", "desiredChildren", "intimacyBoundary"], "서로의 경계를 묻는 대화");
    if (["child_shared_plan", "child_start_now", "child_accept_choice"].includes(id)) successOnly(["childIntent", "desiredChildren"], "아이 계획을 구체적으로 나눈 대화");
    if (id.startsWith("special_")) successOnly(["responseRhythm", "speechTone"], "주인공만의 방식으로 나눈 깊은 대화");
    ...
    if (["shadow_name_it", "shadow_pact"].includes(id) && checkSuccess) {
      revealProfile(["personalityCore", "intimacyBoundary", "jealousy"], "서로 숨기고 싶은 욕구를 말한 대화", result);
    }
  }

  function resolvePlayerSpecial(checkSuccess) {
    const player = getPlayer();
    state.specialUses = (state.specialUses || 0) + 1;
    state.specialLastScene = state.scene;
    const successText = {
      junsu: "내가 이해한 감정을 먼저 말하자 그녀가 틀린 부분을 고쳐 주며 평소보다 긴 이야기를 꺼냈다.",
      minho: "책임질 범위와 한도를 분명히 하되 결정을 재촉하지 않자, 주도적이면서도 안전하다는 인상을 남겼다.",
      taeho: "날짜와 비용을 정리한 화면을 함께 고치며 감정과 사실이 처음으로 같은 대화 안에 놓였다.",
      seongjin: "해 본 사람의 생활 감각으로 가능한 것과 불가능한 것을 나누자 허세보다 현실적인 신뢰가 생겼다.",
      hyunwoo: "나에게도 같은 확인 기준을 적용하자 심문이 아니라 공정한 약속으로 받아들여졌다.",
      dojun: "꾸미지 않은 표정과 주변 소리가 함께 담긴 영상 덕분에 멋진 문장보다 진짜 하루가 전해졌다.",
      yeonggil: "남은 시간을 강조하는 대신 간병·상속·생활 속도까지 먼저 꺼내자, 직진이 도피가 아니라 책임으로 보였다.",
      gwangho: "돈을 다 내겠다고 밀어붙이지 않고 가게와 생활비의 경계를 공개하자 손 큰 허세가 생활력으로 바뀌었다.",
      junyeong: "당장 떠나자는 말 뒤에 귀가 시간과 비용, 거절할 선택지를 붙이자 즉흥성이 무책임으로 보이지 않았다."
    }[player.id];
    const failText = {
      junsu: "말을 너무 조심스럽게 고르다 핵심 답을 미루는 사람처럼 들렸다.",
      minho: "범위를 정하려던 말이 또 하나의 최후통첩처럼 들려 상대가 한발 물러섰다.",
      taeho: "정리표가 상대의 마음을 이해하려는 도구보다 오류를 찾는 보고서처럼 보였다.",
      seongjin: "생활 경험을 말하는 동안 상대의 조건보다 내 방식만 정답처럼 밀어붙였다.",
      hyunwoo: "공정한 원칙을 설명했지만 지금 필요한 감정에는 답하지 못했다.",
      dojun: "솔직한 영상이 아니라 분위기를 바꾸기 위한 연출처럼 보여 중요한 답이 가벼워졌다.",
      yeonggil: "마지막 기회라는 말이 상대에게는 빠른 결정을 강요하는 시한처럼 들렸다.",
      gwangho: "내가 책임진다는 말 뒤에 상대가 따라야 한다는 조건이 숨어 있는 것처럼 들렸다.",
      junyeong: "뜨거운 순간만 강조해 다음 주 생활과 돈은 전혀 생각하지 않는 사람처럼 보였다."
    }[player.id];
    return checkSuccess
      ? { title: `${player.name}만의 방식이 통했다`, text: successText, trust: 4, affection: 3, clarity: 2, days: 1, reaction: partnerLine("pleased") }
      : { title: `${player.name}의 약점까지 함께 드러났다`, text: failText, trust: -4, affection: -3, conflict: 3, stress: 2, days: 1, bad: true, reaction: partnerLine("hurt") };
  }
  ...
  function applyPlayerCompulsion(id, result) {
    const compulsion = getPlayer()?.compulsion;
    const category = behaviorCategory(id);
    if (!compulsion || !category || !compulsion.triggers?.includes(category) || !result) return;
    const count = (state.compulsionHistory || []).length + 1;
    const pressure = count >= 4 ? 18 : count >= 2 ? 12 : 8;
    state.compulsionPressure = clamp((state.compulsionPressure || 0) + pressure, 0, 100);
    state.compulsionHistory = [{ id: compulsion.id, choice: id, scene: scenes[state.scene]?.id, day: state.elapsedDays }, ...(state.compulsionHistory || [])].slice(0, 30);
    result.affection = Number(result.affection || 0) + 3;
    awardScore(55, `강박의 즉시 보상 · ${compulsion.label}`, result, "compulsion");
    result.text += `\n\n강박 발동 · ${compulsion.label}: 이 선택은 유난히 짜릿했고 즉시 만족을 줬다.`;
    if (count >= 3) {
      result.stress = Number(result.stress || 0) + 3;
      result.conflict = Number(result.conflict || 0) + 2;
      result.partnerFatigue = Number(result.partnerFatigue || 0) + 2;
      result.text += ` 그러나 같은 자극을 ${count}번째 반복하면서 후유증이 시작됐다. ${compulsion.aftertaste}`;
    }
  }
  ...
  function startGame() {
    ...
    state = {
      ...
      partnerMatchScore: 0,
      playerShadow: (player.shadowDesires || []).map(item => ({ id: item.id, intensity: item.intensity || 2, expressed: 0, regulated: 0, distorted: 0 })),
      knownPartnerShadows: [],
      shadowResonance: 0,
      shadowRisk: 0,
      shadowHistory: [],
      compulsionPressure: 0,
      compulsionHistory: [],
      trust: 36,
      affection: 34,
      certainty: 7,
      ...
      communicationClarity: clamp((partner.languageLevel || 35) - 55, -60, 45),
      interpreterOn: false,
      interpreterSpend: 0,
      ...
      monthlyLiving: 2850000,
      monthlySharedPersonal: SHARED_PERSONAL_MONTHLY,
      monthlyCrossBorder: CROSS_BORDER_MONTHLY,
      monthlyRemittance: 0,
      monthlyHousing: 0,
      housingAsset: player.housingAsset || 0,
      housingDeposit: 0,
      homeValue: 0,
      mortgageBalance: 0,
      mortgageRate: 0,
    ...
    if (rematchCarry) {
      state.attempt = rematchCarry.attempt;
      state.roundDayOffset = rematchCarry.elapsedDays;
      state.cash = rematchCarry.cash;
      state.debt = rematchCarry.debt;
      state.spent = rematchCarry.spent;
      state.score = rematchCarry.score;
      state.scoreLog = rematchCarry.scoreLog;
      state.stress = rematchCarry.stress;
      state.elapsedDays = rematchCarry.elapsedDays;
      state.daysLeft = rematchCarry.daysLeft;
      state.moneyLog = rematchCarry.moneyLog;
      state.compulsionPressure = rematchCarry.compulsionPressure;
      rematchCarry = null;
    } else {
  ...
  function earn(amount, label) {
    if (!amount) return;
    state.moneyLog.unshift({ label, amount, direction: "income", day: state.elapsedDays });
    state.cash += amount;
  }

  function repayDebt(amount, label = "급전 상환") {
    const paid = Math.min(state.debt || 0, state.cash || 0, Math.max(0, amount || 0));
    if (!paid) return 0;
    state.cash -= paid;
    state.debt -= paid;
    state.moneyLog.unshift({ label, amount: paid, direction: "repayment", day: state.elapsedDays });
    return paid;
  }
  ...
  function incomeFactorFor(player) {
    ...
    if (player.incomeRisk === "business") return roll < .12 ? .2 : roll < .36 ? .62 + Math.random() * .18 : .9 + Math.random() * .34;
    if (player.incomeRisk === "freelance") return roll < .18 ? .25 : roll < .48 ? .55 + Math.random() * .22 : .9 + Math.random() * .4;
    return roll < .06 ? .55 : .88 + Math.random() * .17;
  }
  ...
  function passTime(days) {
    ...
    if (salaryMonths > 0) {
      const player = getPlayer();
      for (let month = 0; month < salaryMonths; month += 1) {
        const playerFactor = incomeFactorFor(player);
        const playerIncome = Math.round(player.income * (state.playerIncomeFactor || 1) * playerFactor);
        const partnerWorkRoll = Math.random();
        const partnerWorkFactor = state.pregnancy ? .48
          : state.children && state.flags.partnerHomeCare ? 0
            : partnerWorkRoll < .1 ? .2
              : partnerWorkRoll < .22 ? .62
                : .88 + Math.random() * .17;
        const partnerIncome = state.married ? Math.round((state.partnerIncome || 0) * partnerWorkFactor) : 0;
        const householdIncome = playerIncome + partnerIncome;
        const incomeLabel = playerFactor < .45 ? `${player.job} 수입 급락`
          : playerFactor < .8 ? `${player.job} 휴직·수입 감소`
            : state.married && partnerIncome > 0 && partnerWorkFactor < .4 ? "배우자 휴직·근무 공백이 반영된 부부 수입"
              : state.married ? "이번 달 부부 수입" : "이번 달 급여";
        earn(householdIncome, incomeLabel);
        if (state.married) {
          const childCost = state.children * 785000;
          const pregnancyCost = state.pregnancy ? 320000 : 0;
          const settlingCost = state.monthsMarried <= 6 ? EARLY_SETTLING_MONTHLY : 0;
          const livingCost = state.monthlyLiving + player.monthlyCommitment + state.monthlyRemittance + childCost + pregnancyCost
            + (state.monthlySharedPersonal || SHARED_PERSONAL_MONTHLY) + (state.monthlyCrossBorder || CROSS_BORDER_MONTHLY) + settlingCost;
          spend(livingCost, `부부 생활비${state.children ? "·양육비" : ""}`);
          if (state.ending) break;
          if (state.monthlyHousing) {
            spend(state.monthlyHousing, state.mortgageBalance ? "주담대 원리금·관리비" : "월세·주거비");
            if (state.ending) break;
          }
          if (state.mortgageBalance > 0 && state.mortgagePayment > 0) {
            const mortgageInterest = Math.round(state.mortgageBalance * state.mortgageRate / 12);
            const principal = Math.max(0, state.mortgagePayment - mortgageInterest);
            state.mortgageBalance = Math.max(0, state.mortgageBalance - principal);
          }
          const housingBurden = householdIncome ? (state.monthlyHousing || 0) / householdIncome : 1;
          if (housingBurden >= .5) {
            state.stress = clamp(state.stress + 6, 0, 100);
            state.conflict = clamp(state.conflict + 3, 0, 100);
            state.partnerFatigue = clamp(state.partnerFatigue + 8, 0, 100);
          } else if (housingBurden >= .35) {
            state.stress = clamp(state.stress + 3, 0, 100);
            state.partnerFatigue = clamp(state.partnerFatigue + 4, 0, 100);
          } else state.partnerFatigue = clamp(state.partnerFatigue - 1, 0, 100);
          if ((state.housingSatisfaction || 0) < 0) {
            state.affection = clamp(state.affection - 1, 0, 100);
            state.conflict = clamp(state.conflict + 1, 0, 100);
          ...
          if (state.partnerFatigue >= 80) {
            ...
            state.conflict = clamp(state.conflict + 2, 0, 100);
          } else if (state.partnerFatigue >= 60) state.affection = clamp(state.affection - 1, 0, 100);
        } else {
        ...
        if (state.debt > 0) {
          ...
          state.stress = clamp(state.stress + debtPressure, 0, 100);
          if (state.married && state.debt >= 6000000) state.conflict = clamp(state.conflict + Math.ceil(debtPressure * .6), 0, 100);
  ...
  function syncSceneCalendar() {
    ...
    if (!id) return;
    if (!state.married) {
      const baseTarget = PRE_MARRIAGE_DAY[id];
      const target = Number.isFinite(baseTarget) ? (state.roundDayOffset || 0) + baseTarget : baseTarget;
  ...
  function phaseName() {
    ...
    if (["housing_choice", "new_home", "household_money", "partner_work", "family_remittance", "intimacy_distance", "intimacy_talk"].includes(id)) return "신혼 생활";
    ...
    switch (scene.id) {
      ...
      case "her_daily_message":
        return { ...common, mood: "먼저 연락해 옴", messages: [
          ...
          choice("daily_short_reply", "“ㅎㅎ 귀엽네요.”", "무난하게 반응하고 내 이야기는 꺼내지 않는다.", "소통 판정 · 성공해도 관계와 정보 변화는 작음", 0, "plain")
        ] };
      case "chat_why":
      ...
      case "arrival":
        return { ...common, mood: "긴장한 웃음", portrait: false, eventCard: true, eventTitle: `${country.city}에 도착했다`, eventText: `왕복 항공권·4박 숙소·현지 데이트 비용이 국가별 대표값으로 반영됩니다. 이제 프로필 사진이 아닌 실제 모습으로 관계가 이어집니다.`, context: `현지 첫 만남 · 공항 도착`, text: `입국장을 나서자 ${partner.name}이 먼저 손을 흔든다. 실제로 보니 ${partner.profileDifference}. 화면 밖의 사람이 눈앞에 있다는 사실에 둘 다 한동안 웃기만 했다.\n\n“직접 보니까 저도 말이 안 나오네요. 우선 밥부터 먹을까요?”`, choices: [
          choice("arrival_direct", "둘이 천천히 식사한다", "왕복 항공·4박 숙소·기본 데이트만 쓰고 통역 없이 쉬운 말로 대화한다.", `지금: ${formatWon(tripQuote("standard"))} · 앞으로: 둘만의 리듬`, tripQuote("standard"), "romance"),
          choice("arrival_interpreter", "독립 통역을 하루 고용한다", "업체 통역사가 아닌 사람에게 중요한 말만 정확히 확인한다.", `지금: ${formatWon(tripQuote("interpreter"))} · 앞으로: 정확한 기억`, tripQuote("interpreter"), "investigate"),
          choice("arrival_luxury", "좋은 호텔과 선물로 분위기를 띄운다", "말이 막힐 때 돈을 써서 첫날의 분위기를 끌어올린다.", `지금: ${formatWon(tripQuote("luxury"))} · 앞으로: 호감 증가, 기대수준 증가`, tripQuote("luxury"), "risky")
        ] };
      case "private_talk":
      ...
      case "documents":
        return { ...common, speaker: "상대 프로필 · 발견 기록", mood: "함께 확인하기", text: `프로필 사진의 원본, 실제 직장, 이전 혼인 여부, 현재 거주지와 돈 계획을 한 항목씩 적었다.\n\n서류를 본다는 것은 상대를 범인 취급하는 일이 아니다. 둘이 같은 사실을 알고 결혼을 판단하기 위한 과정이다. 출처를 알 수 없는 소문은 사실 칸에 넣지 않는다.`, choices: [
      ...
      case "shadow_mirror": {
        const mine = (getPlayer().shadowDesires || []).map(item => item.label).join(" · ");
        const pactText = (state.playerShadow || []).some(item => item.id === "control")
          ? "내가 주도하고 싶은 순간과 그녀가 반드시 직접 정할 부분을 나눈다."
          : (state.playerShadow || []).some(item => item.id === "surrender")
            ? "그녀에게 맡기고 싶은 순간과 내가 직접 책임질 부분을 나눈다."
            : "서로 주도권을 바꿔 보고 불편하면 즉시 멈추는 규칙을 만든다.";
        return { ...common, mood: "말하면 들킬 것 같은 마음", portrait: false, eventCard: true, eventTitle: "좋아함보다 말하기 어려운 것", eventText: `내 그림자 욕구\n${mine || "아직 이름 붙이지 못함"}`, text: `${partner.name}이 한참 침묵하다가 말한다.\n\n“좋은 사람처럼 보이려고 하는 말 말고요. 질투하거나, 상대를 마음대로 하고 싶거나, 반대로 전부 맡기고 싶은 순간도 있잖아요. 그런 마음이 생기면 우리는 어떻게 할까요?”\n\n욕구는 죄가 아니지만, 상대의 선택권을 지우는 순간부터 관계의 위험이 된다.`, choices: [
          choice("shadow_name_it", "내 욕구 하나를 먼저 말하고 그녀의 욕구도 묻는다", "창피한 마음을 포장하지 않되, 그 욕구를 들어줄 의무는 없다고 먼저 말한다.", "공감·동의 판정 · 성공하면 서로의 어두운 욕구 1개 발견", 0, "romance", { stat: "empathy", assist: "courage", base: 54, joint: true, private: true, label: "그림자 대화" }),
          choice("shadow_pact", "주도권과 멈춤 규칙을 작은 행동으로 시험한다", pactText, "배짱·동의 판정 · 공명하면 큰 점수, 맞지 않으면 즉시 중단", 0, "investigate", { stat: "courage", assist: "empathy", base: 50, joint: true, private: true, label: "경계 합의" }),
          choice("shadow_test", "일부러 답장을 늦추고 다른 사람 이야기를 꺼내 질투를 본다", "상대가 얼마나 나를 원하는지 말 대신 불안으로 시험한다.", "반응은 빨리 보임 · 그림자 왜곡과 갈등 크게 증가", 0, "risky"),
          choice("shadow_bury", "“난 그런 욕구 없어”라고 웃으며 덮는다", "좋은 사람처럼 보이는 쪽을 택하고 더 묻지 않는다.", "당장은 안전 · 욕구는 이름 없이 뒤에서 누적", 0, "plain")
        ] };
      }
      case "turning_point":
      ...
      case "investigation":
        return { ...common, speaker: "조사 선택", mood: "서로 다른 기록을 펼침", text: `프로필, 대화, 원본 자료에는 비슷해 보이지만 서로 맞지 않는 문장이 섞여 있다. 먼저 세 쌍 중 실제 모순 하나를 직접 연결해 본다.\n\n현재 남은 일정은 ${Math.max(0, state.daysLeft)}일이다.`, choices: [
          choice("open_contradiction_board", "프로필 모순 연결 보드를 연다", "프로필·대화·원본 자료 세 쌍을 읽고, 서로 직접 어긋나는 한 쌍을 고른다.", "주사위 없음 · 관찰한 내용으로 직접 판단", 0, "evidence"),
          ...investigationChoices(mystery)
        ] };
      case "anonymous_tip":
      ...
      case "final_check":
        return { ...common, mood: "마지막 기회", text: `결정 전날 밤, 상대 프로필의 발견 기록을 다시 펼쳤다. 확인한 사실도 있고 돈과 시간이 없어 아직 모르는 부분도 있다.\n\n마지막으로 한 통만 더 전화할 수 있다. 무엇을 확인할지가 결론보다 중요할 수 있다.`, choices: [
      ...
      case "move_country":
        return { ...common, speaker: partner.name, mood: "현실적인 표정", portrait: false, eventCard: true, eventTitle: "결혼 전에 정해야 할 생활", eventText: "거주·경력·가족 지원·돈 관리를 한꺼번에 협상합니다. 모든 것을 얻는 정답은 없습니다.", context: `결혼 결정 전 · 생활조건 협상`, text: `“결혼부터 하고 나중에 정하면 한 사람은 선택권이 없어질 것 같아요. 저는 ${partner.job}으로 월 약 ${formatWon(partner.estimatedIncome || 0)}을 벌 수 있고, 당신은 ${player.job}으로 월 ${formatWon(player.income)}을 벌죠. 사는 곳뿐 아니라 일, 가족, 돈까지 지금 같이 정해요.”\n\n내 직업 이동성은 ${player.portability}/10, 그녀의 직업 이동성은 ${partner.jobPortability || 5}/10이다.`, choices: [
          choice("open_negotiation", "결혼조건 협상판을 연다", "네 항목에서 내가 제안할 조건을 골라 그녀의 우선순위와 타협점을 찾는다.", "주사위 없음 · 안정 합의, 비싼 타협, 원한이 남은 절충, 결혼 유예", 0, "investigate")
        ] };
      case "housing_choice": {
        const isLocal = state.homeCountry === "local";
        const isDistance = state.homeCountry === "distance";
        const marketTitle = isLocal ? `${country.name} 1년 거주 견적` : isDistance ? "두 나라 6개월 생활 견적" : "서울 신혼집 견적";
        const marketRule = isLocal
          ? `${country.name} 게임상 생활비 지수 · 가구 포함 임대 기준`
          : isDistance
            ? "항공·두 집 고정비 · 실제로 함께 지내는 시간 반영"
            : `${HOUSING_POLICY.asOf} 게임 데이터\nLTV 최대 70% · DSR 40% · 수도권 주담대 상한 반영`;
        const intro = isLocal
          ? `${country.name}에서 1년을 살아 보기로 한 합의에 맞춰 그녀의 생활권 안에서 집을 고른다. 내 수입은 이동 가능성에 따라 줄고, 현지 임대료와 체류비는 게임상 생활비 지수로 계산한다. 1년 뒤 두 사람은 거주지를 다시 결정한다.`
          : isDistance
            ? `6개월 동안 두 나라를 오가기로 한 합의에 맞춰 왕래 빈도와 두 집의 고정비를 고른다. 싼 선택은 함께 사는 시간이 줄고, 자주 만나는 선택은 항공료와 체류 공간이 가계를 압박한다.`
            : `서울에서 같이 살 집을 고른다. 배우자가 바로 취업하지 못하면 당분간 한 사람 수입으로 버텨야 한다. 매매 대출은 집값의 70%가 자동으로 나오는 것이 아니라 DSR과 상품 한도 중 가장 낮은 값으로 계산한다.`;
        return { ...common, speaker: "주거·생활 상담 화면", mood: "숫자는 낭만을 봐주지 않음", portrait: false, eventCard: true, eventTitle: marketTitle, eventText: marketRule, context: `결혼 ${Math.max(1, state.monthsMarried)}개월째 · 주거 계약 전`, text: `${intro}\n\n현재 현금은 ${formatWon(state.cash)}, 별도 주거 종잣돈은 ${formatWon(state.housingAsset || 0)}이다. 마련할 수 없는 자기자본이 필요한 집은 확률 대신 ‘선택 불가’로 표시된다.`, choices: housingChoices() };
      }
      case "new_home":
        return { ...common, mood: "낯선 집에서 지침", context: `${state.homeCountry === "korea" ? "한국" : state.homeCountry === "local" ? country.name : "두 나라"} 생활 첫 달`, text: `${state.housingChoice ? `선택한 집의 월 고정 주거비는 ${formatWon(state.monthlyHousing || 0)}이다. ` : "임시 거처에서 "}여행 때는 웃고 넘겼던 차이가 매일 반복되기 시작했다. 음식 냄새, 연락 빈도, 난방 온도, 혼자 있고 싶은 시간까지 모두 생활 문제가 됐다.\n\n“${partnerLine("married", "가끔은 해결책보다 제 편이라는 말이 먼저 필요해요.")}”`, choices: [
  ...
  function annuityPayment(principal, annualRate, years = HOUSING_POLICY.termYears) {
  ...
  function principalForPayment(payment, annualRate, years = HOUSING_POLICY.termYears) {
    if (payment <= 0) return 0;
    const monthlyRate = annualRate / 12;
    const months = years * 12;
    return Math.round(payment * ((1 + monthlyRate) ** months - 1) / (monthlyRate * (1 + monthlyRate) ** months));
  }

  function housingQuote(option) {
    const player = getPlayer();
    const monthlyHouseholdIncome = player.income * (state.playerIncomeFactor || 1) + Math.max(0, state.partnerIncome || 0);
    const availableFunds = (state.housingAsset || 0) + Math.max(0, state.cash);
    if (option.tenure === "rent") {
      const shortfall = Math.max(0, option.deposit - availableFunds);
      const burden = monthlyHouseholdIncome ? option.monthly / monthlyHouseholdIncome : 1;
      return {
        availableFunds,
        shortfall,
        hardBlocked: shortfall > 0,
        monthly: option.monthly,
        burden,
        approvalChance: shortfall ? 0 : clamp(76 - Math.round(burden * 24) + (option.fit || 0) * 2, 42, 86),
        detail: `보증금 ${formatWon(option.deposit)} · 월 ${formatWon(option.monthly)}`
      };
    }
    const policyEligible = option.price <= HOUSING_POLICY.policyHomeCap && player.income * 12 <= 85000000;
    const rate = policyEligible ? HOUSING_POLICY.policyRate : HOUSING_POLICY.bankRate;
    const monthlyDsrRoom = Math.max(0, monthlyHouseholdIncome * HOUSING_POLICY.dsr - player.monthlyCommitment);
    const dsrCap = principalForPayment(monthlyDsrRoom, rate);
    const productCap = policyEligible ? HOUSING_POLICY.policyLoanCap : HOUSING_POLICY.mortgageCap;
    const loan = Math.max(0, Math.min(option.price * HOUSING_POLICY.ltv, productCap, dsrCap));
    const closing = Math.round(option.price * .04);
    const requiredFunds = option.price - loan + closing;
    const shortfall = Math.max(0, requiredFunds - availableFunds);
    const monthly = annuityPayment(loan, rate) + (option.area >= 79 ? 420000 : 260000);
    const burden = monthlyHouseholdIncome ? monthly / monthlyHouseholdIncome : 1;
    return {
      availableFunds,
      policyEligible,
      rate,
      ...
      monthly,
      burden,
      approvalChance: shortfall ? 0 : clamp(74 - Math.round(burden * 30) + (option.fit || 0) * 2, 38, 82),
      detail: `${formatWon(option.price)} · 대출 ${formatWon(loan)} · 월 ${formatWon(monthly)}`
    };
  }

  function housingChoices() {
    return housingOptions().map(option => {
      const quote = housingQuote(option);
      const shortage = quote.shortfall ? ` · 자기자본 ${formatWon(quote.shortfall)} 부족` : "";
      return choice(
        `housing_${option.id}`,
        option.name,
        `${option.area}㎡ · 방 ${option.bedrooms}개. ${option.note}`,
        `${quote.detail}${shortage} · 예상 월부담률 ${Math.round(quote.burden * 100)}%${quote.hardBlocked ? " · 현재 선택 불가" : ""}`,
        0,
        quote.shortfall ? "risky" : option.tenure === "buy" ? "investigate" : "romance",
        { stat: "reason", base: quote.approvalChance, joint: true, housing: true, hardBlocked: quote.hardBlocked, label: quote.hardBlocked ? "자기자본 부족 · 선택 불가" : "두 사람의 생활 합의" }
      );
    });
  }

  function takeHousingFunds(amount, label) {
    const fromAsset = Math.min(state.housingAsset || 0, amount);
    state.housingAsset = Math.max(0, (state.housingAsset || 0) - fromAsset);
    const fromCash = amount - fromAsset;
    if (fromCash > 0) state.cash = Math.max(0, state.cash - fromCash);
    state.moneyLog.unshift({ label, amount, direction: "housing", day: state.elapsedDays });
  }

  function housingPartnerFit(option, quote) {
    const behavior = getPartner().behavior?.id;
    const desiredChildren = getPartner().privateTraits?.desiredChildren || 0;
    let fit = option.fit || 0;
    if (behavior === "family_centered" && option.bedrooms >= 2) fit += 2;
    if (["practical_planner", "guarded_survivor"].includes(behavior) && quote.burden <= .38) fit += 2;
    if (["passionate_impulsive", "playful_social"].includes(behavior) && /아파트/.test(option.name)) fit += 1;
    if (desiredChildren >= 3 && option.bedrooms < 3) fit -= 3;
    if (quote.burden >= .5) fit -= 3;
    return clamp(fit, -6, 6);
  }

  function resolveHousingChoice(id, checkSuccess) {
    const option = housingOptions().find(item => `housing_${item.id}` === id);
    if (!option) return null;
    const quote = housingQuote(option);
    if (quote.hardBlocked) return null;
    if (!checkSuccess) {
      const fallback = housingFallback();
      const fallbackQuote = housingQuote(fallback);
      const deposit = fallback.deposit || 0;
      takeHousingFunds(deposit, "임시 신혼집 보증금");
      state.housingDeposit = deposit;
      state.monthlyHousing = fallback.monthly;
      state.housingChoice = fallback.id;
      state.housingSatisfaction = -3;
      return {
        title: "집보다 먼저 생활 합의가 막혔다",
        text: `돈은 마련할 수 있었지만 위치·공간·월 부담 중 하나에 합의하지 못했다. 선택했던 집을 억지로 계약하지 않고 ${fallback.name}에서 한 달만 지내며 다시 고르기로 했다. 월 ${formatWon(fallbackQuote.monthly)}이 임시 고정비가 된다.`,
        cost: 250000,
        costLabel: "임시 이사·보관비",
        affection: -3,
        trust: -2,
    ...
    const fit = housingPartnerFit(option, quote);
    if (option.tenure === "rent") {
      takeHousingFunds(option.deposit, `${option.name} 보증금`);
      state.housingDeposit = option.deposit;
      state.monthlyHousing = option.monthly;
    } else {
      takeHousingFunds(quote.requiredFunds, `${option.name} 자기자본·취득비`);
      state.homeValue = option.price;
      state.mortgageBalance = quote.loan;
      state.mortgageRate = quote.rate;
      state.mortgagePayment = annuityPayment(quote.loan, quote.rate);
      state.monthlyHousing = quote.monthly;
    }
    state.housingChoice = option.id;
    state.housingSatisfaction = fit;
    const reaction = fit >= 3 ? "기대했던 생활에 가까워졌다며 방을 몇 번이나 둘러봤다." : fit >= 0 ? "완벽하진 않지만 감당 가능한 선택이라며 가구 배치부터 의논했다." : "집 자체보다 앞으로 매달 버틸 돈과 좁은 공간을 걱정했다.";
    return {
      title: `${option.name}에서 신혼을 시작한다`,
      text: `${quote.detail}. ${HOUSING_POLICY.asOf} 게임 데이터 기준으로 계산한 결과다. ${reaction}`,
      affection: fit,
      trust: fit >= 0 ? 4 : -2,
      conflict: fit < 0 ? Math.abs(fit) : -1,
      stress: quote.burden >= .45 ? 8 : quote.burden >= .33 ? 3 : -2,
      partnerFatigue: quote.burden >= .45 ? 8 : fit < 0 ? Math.abs(fit) + 2 : -3,
      reaction: partnerLine(fit >= 0 ? "pleased" : "money")
    };
  }
  ...
  function weddingPlaceChoices() {
    const country = getCountry();
    return [
      choice("wedding_register", "혼인신고를 먼저 하고 식은 1년 뒤에 할까?", "지금은 두 가족에게 작은 식사와 영상 인사만 하고, 생활비 비상금이 생긴 뒤 예식을 다시 정한다.", "합의 판정 · 80만원 · 현금 방어, 가족의 아쉬움", 800000, "investigate", { stat: "empathy", assist: "reason", base: 66, joint: true, label: "예식 연기 합의" }),
  ...
  function weddingBudgetChoices() {
    return [
      choice("wedding_defer_party", "사진과 가족 식사만 남긴다", "큰 잔치는 약속한 시점까지 미루고, 양가에게 같은 설명과 예산표를 보낸다.", "추가 30만원 · 주거 비상금 보존, 가족 서운함", 300000, "investigate"),
  ...
  function anniversaryContent(common) {
    return { ...common, speaker: getPartner().name, mood: "마지막 답을 기다림", context: `결혼 ${Math.max(1, state.monthsMarried)}개월째 · 식탁 위에 놓인 발견 기록`, text: `“우리가 처음 만났을 때부터 당신은 진짜 사랑인지 사기인지 계속 물었죠. 이제는 저도 묻고 싶어요. 함께 산 시간을 보고도 나와 계속 살고 싶어요?”\n\n결혼 전 자료와 결혼 뒤 생활 기록을 함께 펼쳤다. 마지막 확인을 할 수도 있고, 지금까지의 설명을 믿을 수도 있다.`, choices: [
  ...
  function familyDecisionContent(common) {
    ...
    return { ...common, speaker: "나", mood: "함께 산 시간을 돌아봄", context: `결혼 ${Math.max(1, state.monthsMarried)}개월째 · 최종 선택`, portrait: state.children === 0, bg: state.children ? "newborn" : "romance-start", text: `결혼식 이후 ${Math.max(1, state.monthsMarried)}개월을 함께 살았다. 아이는 ${state.children}명, 남은 급전은 ${formatWon(state.debt)}, 주담대는 ${formatWon(state.mortgageBalance || 0)}, 갈등은 ${state.conflict}/100이다. 상대 프로필의 발견 기록에는 확인된 사실 ${facts}개가 남았다.\n\n이제 결혼 전의 약속이 아니라 실제로 함께 산 결과를 놓고 다음 시기를 정한다.`, choices: [
  ...
  function openInvestigationPlanner(mode) {
    ...
    openModal(field ? "현지 조사 계획" : "디지털 확인 계획", "무엇부터 확인할까요?", `<p class="warning-box">한 번에 모든 것을 알 수 없습니다. 현재 의심과 직접 연결된 대상을 골라야 비용과 시간이 의미가 있습니다.</p><div class="investigation-grid">${options.map(([id, title, desc]) => `<button class="investigation-option" data-investigation-target="${id}" type="button"><strong>${title}</strong><span>${desc}</span><small>${formatWon(cost)} · ${field ? "5일" : "3일"}</small></button>`).join("")}</div>`, element => {
  ...
  function interrogationContent(index) {
    ...
    return {
      ...
      choices: [
        ...
        choice(`present_${index}`, "이 설명과 맞지 않는 자료를 보여 준다", "상대 프로필의 확인 자료에서 방금 한 말과 직접 어긋나는 항목을 고른다.", "성공: 앞뒤가 다른 말 확인 · 실패: 신뢰 -12, 침착함 -1", 0, "evidence"),
  ...
  function inferredCheck(item) {
    ...
    const stat = item.style === "investigate" ? "reason"
      : item.style === "risky" ? "courage"
        : category === "humor" ? "charm"
          : category === "practical" ? "reason"
            : "empathy";
    const base = item.style === "risky" ? 48 : item.style === "investigate" ? 58 : 60;
    return { stat, base, auto: true, label: `${statLabel(stat)}·소통` };
  }

  function playerSpecialChoice() {
    const player = getPlayer();
    const scene = scenes[state.scene];
    const allowedScenes = new Set(["private_talk", "her_question", "money_crisis", "breathing_room", "her_investigation", "marriage_crisis"]);
    if (!player || !scene || !allowedScenes.has(scene.id) || (state.specialUses || 0) >= 3 || state.scene - (state.specialLastScene ?? -99) < 3) return null;
    const specials = {
      junsu: { stat: "empathy", title: "[준수 전용] 그녀의 말을 내 말로 다시 확인한다", description: "해결책을 내기 전에 내가 이해한 감정과 요구가 맞는지 조심스럽게 되묻는다.", impact: "성공하면 신뢰와 성격 정보 · 실패하면 우유부단하게 들림" },
      minho: { stat: "courage", title: "[민호 전용] 내가 책임질 범위와 마감부터 제시한다", description: "큰소리 대신 돈·시간·책임의 상한을 숫자로 말하고 즉답을 요구하지 않는다.", impact: "성공하면 주도권과 신뢰 · 실패하면 압박으로 받아들임" },
      taeho: { stat: "reason", title: "[태호 전용] 말·날짜·비용을 한 화면에 맞춘다", description: "감정을 무시하지 않는다는 전제로 서로 기억하는 사실을 짧은 타임라인으로 정리한다.", impact: "성공하면 확인도와 생활 합의 · 실패하면 취조처럼 보임" },
      seongjin: { stat: "courage", title: "[성진 전용] 실제 살림 기준으로 가능한 답을 낸다", description: "차량·근무·생활비 경험을 바탕으로 오늘 할 일과 못 할 일을 솔직하게 나눈다.", impact: "성공하면 현실 신뢰 · 실패하면 돈으로 밀어붙인 인상" },
      hyunwoo: { stat: "reason", title: "[현우 전용] 나에게도 같은 원칙을 적용한다", description: "상대에게 요구한 확인 자료와 생활 규칙을 내 쪽에도 똑같이 적용하겠다고 약속한다.", impact: "성공하면 공정성·신뢰 · 실패하면 규정만 말하는 사람처럼 보임" },
      dojun: { stat: "charm", title: "[도준 전용] 꾸미지 않은 짧은 영상으로 답한다", description: "멋진 말보다 지금 있는 장소와 표정을 보여 주며 생활 속의 솔직한 답을 보낸다.", impact: "성공하면 호감과 생활 대화 · 실패하면 가벼운 연출로 보임" },
      yeonggil: { stat: "courage", title: "[영길 전용] 황혼의 현실까지 먼저 꺼낸다", description: "설렘만 말하지 않고 건강·상속·간병·남은 시간의 차이를 숨김없이 대화한다.", impact: "맞으면 책임 있는 직진 · 어긋나면 결혼 시한 압박" },
      gwangho: { stat: "reason", title: "[광호 전용] 가게 돈과 부부 돈을 칼같이 나눈다", description: "손 크게 약속하기 전에 사업 고정비와 실제 생활비 상한을 공개한다.", impact: "맞으면 생활 신뢰 · 어긋나면 또 다른 통제 규칙" },
      junyeong: { stat: "charm", title: "[준영 전용] 즉흥 제안에 안전장치를 붙인다", description: "당장 떠나자는 설렘에 비용·귀가·거절 선택지를 함께 제시한다.", impact: "맞으면 자유로운 설렘 · 어긋나면 무책임한 속도" }
    };
    const special = specials[player.id];
  ...
  function isPrivateInterpreterCheck(check) {
    const privateScenes = new Set(["first_intimacy", "shadow_mirror", "intimacy_distance", "intimacy_talk", "marriage_crisis"]);
    return Boolean(check?.private || privateScenes.has(scenes[state.scene]?.id));
  }

  function communicationModifier(check) {
    if (!check || check.stat === "fertility" || check.housing) return 0;
    const weight = ["empathy", "charm", "courage"].includes(check.stat) || check.joint ? .42 : .28;
    return Math.round((state.communicationClarity || 0) * weight);
  }

  function interpreterBonus(check, enabled = state.interpreterOn) {
    if (!enabled || !check || check.stat === "fertility" || check.housing) return 0;
    if (isPrivateInterpreterCheck(check)) return 6;
    const highStakes = /contract|money|wedding|move_|child_|health|interrogation|official|decision/.test(scenes[state.scene]?.id || "");
    const base = highStakes ? 25 : 16;
    return clamp(Math.round(base + Math.max(0, -(state.communicationClarity || 0)) * .2), highStakes ? 22 : 12, highStakes ? 34 : 25);
  }

  function chatBubbleHtml(message, index, keyPrefix) {
  ...
  function renderGame() {
    ...
    if ($("#score-label")) $("#score-label").textContent = Number(state.score || 0).toLocaleString("ko-KR");
    if ($("#communication-label")) $("#communication-label").textContent = `${state.communicationClarity >= 0 ? "+" : ""}${state.communicationClarity}`;
    [$("#interpreter-toggle"), $("#mobile-interpreter")].filter(Boolean).forEach(button => {
      button.classList.toggle("is-on", Boolean(state.interpreterOn));
      button.setAttribute("aria-pressed", String(Boolean(state.interpreterOn)));
      button.innerHTML = button.id === "mobile-interpreter"
        ? `<span>🗣️</span>통역 ${state.interpreterOn ? "ON" : "OFF"}`
        : `🗣️ 전문 통역 ${state.interpreterOn ? "ON" : "OFF"}<small>선택할 때만 비용 차감</small>`;
    });
    ...
    renderChoices(enhanceSceneChoices(content.choices));
    updateSidebar();
    saveGame(false);
  }
  ...
  function renderChoices(choices) {
    ...
    choices.forEach(item => {
      ...
      if (item.special) node.classList.add("is-player-special");
      ...
      node.querySelector(".choice-impact").textContent = `${impact}${choiceFitHint(item.id)}`;
      node.querySelector(".choice-cost").textContent = item.cost ? formatWon(item.cost) : "";
      const translationCost = item.check ? interpreterCost(item) : 0;
      if (item.check) {
        const chanceNode = node.querySelector(".choice-chance");
        chanceNode.hidden = false;
        if (worldCheck) {
          const chance = chanceFor(item.check, state.interpreterOn);
          chanceNode.textContent = item.check.hardBlocked ? item.check.label : `세계 변수 ${chance}% · 결과는 무작위`;
      ...
      if (item.check?.hardBlocked) {
        node.disabled = true;
        node.setAttribute("aria-disabled", "true");
      }
      const projectedDebt = Math.max(0, (item.cost || 0) + (state.interpreterOn ? translationCost || 0 : 0) - state.cash);
      if (projectedDebt && state.debt + projectedDebt >= DEBT_LIMIT) {
        node.querySelector(".choice-cost").textContent += " · 파산 위험";
      }
      if (!item.check?.hardBlocked) node.addEventListener("click", () => handleChoice(item));
  ...
  function updateSidebar() {
    ...
    $("#side-days").textContent = state.married ? `결혼 ${state.monthsMarried}개월` : `교제 ${state.elapsedDays + 1}일 · 결정까지 ${Math.max(0, state.daysLeft)}일`;
    const knownCount = Object.keys(state.knownProfile || {}).length + (state.revealedSecrets || []).length;
    const unread = Number(state.profileUnread || 0);
    if ($("#partner-known-count")) $("#partner-known-count").textContent = unread ? "NEW" : `${knownCount}개`;
    if ($("#hud-known-count")) $("#hud-known-count").textContent = unread ? "NEW" : `${knownCount}개`;
    ["#open-partner-profile", "#open-partner-profile-hud", "#mobile-partner"].forEach(selector => $(selector)?.classList.toggle("has-new", unread > 0));
  ...
  function isWorldCheck(item) {
    if (!item?.check) return false;
    return Boolean(item.check.world || item.check.stat === "fertility" || /business|flight|weather|lottery|pregnancy_roll/.test(item.id));
  }

  function deterministicReadScore(item) {
    const partner = getPartner();
    const behavior = partner?.behavior || {};
    const category = behaviorCategory(item.id);
    let score = ((item.check?.base || 52) - 50) / 8;
    if (category && behavior.likes?.includes(category)) score += 3;
    if (category && behavior.dislikes?.includes(category)) score -= 3;
    const reaction = partner?.hiddenReaction;
    const reactionFits = {
      cautious: ["honest", "space", "care"],
      approval: ["humor", "affection", "brag"],
      planner: ["practical", "honest", "ambition"],
      observer: ["space", "honest", "care"],
      family: ["family", "care", "affection"],
      independent: ["ambition", "honest", "practical"],
      impulsive: ["adventure", "affection", "humor", "brag"],
      guarded: ["honest", "practical", "care"]
    };
    const reactionClashes = {
      cautious: ["pressure", "control"],
      approval: ["cold", "delay"],
      planner: ["brag", "pressure"],
      observer: ["pressure", "interrogation"],
      family: ["cold", "control"],
      independent: ["control", "pressure"],
      impulsive: ["cold", "delay"],
      guarded: ["brag", "pressure", "interrogation"]
    };
    if (category && reactionFits[reaction]?.includes(category)) score += 2;
    if (category && reactionClashes[reaction]?.includes(category)) score -= 2;
    if (item.check?.stat) score += clamp((statValue(item.check.stat) - 5) * .42, -2, 2);
    if (item.check?.assist) score += clamp((statValue(item.check.assist) - 5) * .18, -.8, .8);
    score += clamp(communicationModifier(item.check) / 12, -2.5, 2.5);
    if (state.interpreterOn) score += clamp(interpreterBonus(item.check, true) / 10, 0, 2.5);
    if (item.check?.joint) score += (state.trust - 45) / 18 + (state.affection - 45) / 28 - state.conflict / 26;
    if (state.stress >= 75) score -= 1.2;
    const observed = state.behaviorObservations?.[category] || 0;
  ...
  function readPartnerReaction(item) {
    const score = deterministicReadScore(item);
    const success = score >= 0;
    state.lastRead = {
      scene: state.scene,
      choice: item.id,
      category: behaviorCategory(item.id),
      mbti: getPartner()?.mbti,
      success
    };
    state.pendingSkillText = success && item.check?.stat ? gainSkillXp(item.check.stat) : "";
    return completeChoice(item.id, success);
  }

  function chanceFor(check, interpreterEnabled = state.interpreterOn) {
    if (check.hardBlocked) return 0;
    ...
    const communication = communicationModifier(check);
    const interpreted = interpreterBonus(check, interpreterEnabled);
    return clamp(Math.round(check.base + main + assist + relationship + jointDecision + appearanceSupport + residenceFit + japanAgePenalty + stressPenalty + communication + interpreted), 5, 95);
  }
  ...
  function runRoll(item) {
    ...
    $("#roll-rule").textContent = `세계 변수 · ${chance}% 가능성`;
  ...
  function warningFor(item) {
    ...
    const extraCost = state.interpreterOn ? interpreterCost(item) : 0;
    const newDebt = state.debt + Math.max(0, (item.cost || 0) + extraCost - state.cash);
  ...
  function handleChoice(itemOrId, confirmed = false) {
    ...
    state.lastChoiceTitle = item.title || "";
    state.pendingChoiceMeta = item;
    state.pendingInterpreterUsed = false;
    const translationCost = state.interpreterOn ? interpreterCost(item) : 0;
    if (translationCost) {
      spend(translationCost, `전문 통역 · ${item.title}`);
      if (state.ending) return;
      state.interpreterSpend = (state.interpreterSpend || 0) + translationCost;
      state.pendingInterpreterUsed = true;
    }
    if (item.check) return isWorldCheck(item) ? runRoll(item) : readPartnerReaction(item);
    return completeChoice(id, true);
  }

  function statSnapshot() {
    return { affection: state.affection, trust: state.trust, certainty: state.certainty, conflict: state.conflict, stress: state.stress, partnerFatigue: state.partnerFatigue };
  }

  function statChangesSince(before) {
    const labels = { affection: "호감", trust: "그녀의 신뢰", certainty: "사실 확인도", conflict: "갈등", stress: "스트레스", partnerFatigue: "상대 피로" };
    return Object.entries(labels).map(([key, label]) => ({ key, label, amount: state[key] - before[key] })).filter(item => item.amount !== 0);
  }

  function contradictionBoardRows() {
    const mystery = getCase();
    const route = getRoute();
    const statement = mystery.statements?.[0];
    const sourceClue = firstMissingCaseClue(mystery) || Object.values(mystery.clues || {})[0];
    const suspectClaim = mystery.culprit === "broker"
      ? "업체 설명: 계약서 밖의 비용도 전부 상대 가족에게 전달된다."
      : ["partner", "both"].includes(mystery.culprit)
        ? "프로필 설명: 혼인·직업·돈의 흐름은 처음 소개와 모두 일치한다."
        : "요약 번역: 상대는 원본 확인을 거부하고 오늘 바로 돈을 보내 달라고 했다.";
    const real = {
      profile: suspectClaim,
      conversation: statement?.text || "당사자는 요약문과 실제로 한 말이 다르다고 설명했다.",
      original: `${sourceClue?.title || "원본 자료"}: ${sourceClue?.text || "요약문과 다른 날짜와 발신자가 확인된다."}`,
      clue: sourceClue
    };
    const decoys = [
      {
        profile: "프로필: 주말에는 가까운 가족과 식사하는 편이다.",
        conversation: "“토요일 저녁에는 부모님과 밥을 먹었어요.”",
        original: "공유 달력과 사진 원본 모두 토요일 가족 식사로 기록돼 있다."
      },
      {
        profile: "관심사: 커피와 조용한 카페.",
        conversation: "“퇴근 뒤 카페에서 에스프레소를 마셨어요.”",
        original: "같은 날 카페 영수증과 원본 사진의 촬영 장소가 일치한다."
      }
    ];
    const correctIndex = [...mystery.id].reduce((sum, char) => sum + char.charCodeAt(0), 0) % 3;
    const rows = decoys.slice();
    rows.splice(correctIndex, 0, real);
    return { rows, correctIndex };
  }

  function openContradictionBoard() {
    const { rows, correctIndex } = contradictionBoardRows();
    const body = `<div id="contradiction-board" class="contradiction-board"><p class="board-guide">세 줄을 함께 읽으세요. 분위기가 수상한 항목이 아니라, <strong>한 문장으로 동시에 참일 수 없는 기록</strong>을 고르면 됩니다.</p><div class="contradiction-pairs">${rows.map((row, index) => `<button class="contradiction-pair" data-contradiction-index="${index}" type="button"><span><b>프로필</b>${escapeHtml(row.profile)}</span><span><b>대화</b>${escapeHtml(row.conversation)}</span><span><b>원본</b>${escapeHtml(row.original)}</span><small>이 세 기록을 모순으로 연결</small></button>`).join("")}</div></div>`;
    openModal("모순 연결 보드", "서로 맞지 않는 기록은 어느 쌍일까?", body, element => {
      element.querySelectorAll("[data-contradiction-index]").forEach(button => button.addEventListener("click", () => {
        const selectedIndex = Number(button.dataset.contradictionIndex);
        closeModal();
        resolveContradiction(selectedIndex, correctIndex, rows[selectedIndex]);
      }));
    });
  }

  function resolveContradiction(selectedIndex, correctIndex, selectedRow) {
    const beforeStats = statSnapshot();
    state.choicesMade = [...(state.choicesMade || []), { id: "contradiction_link", scene: scenes[state.scene]?.id, day: state.elapsedDays, success: selectedIndex === correctIndex }].slice(-80);
    state.lastChoiceTitle = "세 기록을 직접 연결했다";
    if (selectedIndex === correctIndex) {
      state.confirmed += 1;
      state.contradictionSolved = true;
      state.trust = Math.min(trustCeiling(), state.trust + balancedTrust(2));
      passTime(2);
      const saved = selectedRow?.clue ? addEvidence(selectedRow.clue) : false;
      if (!saved) gainCertainty(12);
      const result = {
        title: "세 기록이 한 지점에서 어긋났다",
        text: `${selectedRow.profile}\n${selectedRow.conversation}\n${selectedRow.original}\n\n인상이나 국적이 아니라 같은 항목의 말과 원본을 연결해 확인할 질문을 만들었다.`,
        reaction: "어느 부분이 다른지 구체적으로 말해 주자, 그녀도 추측 대신 원본을 놓고 이야기하겠다고 답했다.",
        noApply: true,
        statChanges: statChangesSince(beforeStats)
      };
      awardScore(220, "모순 연결 보드 해결", result, "evidence");
    ...
    passTime(1);
    return showResult({
      title: "다른 표현을 모순으로 착각했다",
      text: `${selectedRow.profile}\n${selectedRow.conversation}\n${selectedRow.original}\n\n세 기록은 표현만 다를 뿐 함께 참일 수 있었다. 잘못된 지목은 새로운 정보 대신 경계만 남겼다.`,
      reaction: "그녀는 같은 날짜와 장소가 적혀 있는데 왜 거짓말이라고 결론 냈는지 되물었다.",
      bad: true,
      noApply: true,
      statChanges: statChangesSince(beforeStats)
    });
  ...
  function negotiationFields() {
    ...
    return [
      {
        id: "residence", label: "첫 거주지", question: "누가 생활권을 옮길까?",
        options: [
          { id: "korea", label: "한국 정착 + 정착지원", note: "그녀의 초기 경력 공백과 한국어 비용을 감수", cost: 350000 },
          { id: "local", label: `${country.name}에서 1년`, note: "내 수입 감소를 감수하고 그녀의 생활권에서 시험", cost: 900000 },
          { id: "distance", label: "6개월 장거리", note: "두 직업을 지키되 왕래 규칙과 재협상 날짜를 확정", cost: 600000 }
        ]
      },
      {
        id: "career", label: "경력", question: "결혼 뒤 누구의 일을 먼저 지킬까?",
        options: [
          { id: "both", label: "두 사람 경력을 모두 유지", note: "수입은 지키지만 일정·교육비 부담이 큼", cost: 500000 },
          { id: "partner_pause", label: "그녀가 6개월 쉬기", note: "정착은 빠르지만 의존과 경력 단절 위험", cost: 150000 },
          { id: "player_adjust", label: "내가 근무를 줄이기", note: "그녀의 경력을 지키되 내 수입이 줄어듦", cost: 1200000 }
        ]
      },
      {
        id: "family", label: "가족 지원", question: "양가 지원의 선은 어디일까?",
        options: [
          { id: "cap", label: "월 30만원 상한", note: "긴급 상황은 다시 협의", cost: 150000 },
          { id: "flexible", label: "월 70만원 + 긴급비", note: "가족 안심은 크지만 가계 부담 증가", cost: 600000 },
          { id: "separate", label: "각자 수입에서 별도", note: "자율성은 높지만 공동 목표와 충돌 가능", cost: 100000 }
        ]
      },
      {
        id: "money", label: "돈 관리", question: "누가 어떤 돈을 볼 수 있을까?",
        options: [
          { id: "three", label: "공동·개인·비상금 3계좌", note: "공동비는 공개하고 개인 돈은 각자 관리", cost: 120000 },
          { id: "player_control", label: "내가 생활비 전부 관리", note: "간단하지만 상대가 허락을 구하는 구조가 됨", cost: 50000 },
          { id: "separate", label: "완전 분리 + 월말 정산", note: "독립성은 높지만 매번 계산해야 함", cost: 250000 }
        ]
      }
    ];
  }

  function negotiationPreferences() {
    const reaction = getPartner()?.hiddenReaction || "cautious";
    return {
      residence: ({ family: "local", independent: "local", planner: "distance", observer: "distance", cautious: "distance", guarded: "distance", approval: "korea", impulsive: "korea" })[reaction] || "distance",
      career: ({ independent: "both", planner: "both", observer: "both", cautious: "both", guarded: "both", family: "partner_pause", approval: "partner_pause", impulsive: "player_adjust" })[reaction] || "both",
      family: ({ family: "flexible", approval: "flexible", impulsive: "flexible", independent: "separate", planner: "cap", observer: "cap", cautious: "cap", guarded: "cap" })[reaction] || "cap",
      money: ({ independent: "separate", impulsive: "separate", family: "three", approval: "three", planner: "three", observer: "three", cautious: "three", guarded: "three" })[reaction] || "three"
    };
  }

  function openNegotiationBoard() {
    const fields = negotiationFields();
    const body = `<div id="negotiation-board" class="negotiation-board"><p class="board-guide">각 항목에는 비용과 후유증이 있습니다. 그녀의 성격과 지금까지의 대화를 읽어 네 조건을 한 번에 제안하세요.</p>${fields.map(field => `<label class="negotiation-group"><span><b>${escapeHtml(field.label)}</b><small>${escapeHtml(field.question)}</small></span><select data-negotiation-field="${field.id}">${field.options.map(option => `<option value="${option.id}">${escapeHtml(option.label)} · ${formatWon(option.cost)}</option>`).join("")}</select><em data-negotiation-note="${field.id}">${escapeHtml(field.options[0].note)}</em></label>`).join("")}<div class="negotiation-total"><span>선택한 조건은 결혼 뒤 실제 수입·송금·주거비에 이어집니다.</span><strong id="negotiation-cost"></strong></div><button id="confirm-negotiation" class="primary-button" type="button">이 조건으로 제안한다</button></div>`;
    openModal("결혼조건 협상", "모든 것을 얻는 정답은 없다", body, element => {
      const refresh = () => {
        let total = 0;
        fields.forEach(field => {
          const select = element.querySelector(`[data-negotiation-field="${field.id}"]`);
          const option = field.options.find(item => item.id === select.value) || field.options[0];
          total += option.cost;
          element.querySelector(`[data-negotiation-note="${field.id}"]`).textContent = option.note;
        });
        element.querySelector("#negotiation-cost").textContent = `준비·상담 ${formatWon(total)}`;
      };
      element.querySelectorAll("[data-negotiation-field]").forEach(select => select.addEventListener("change", refresh));
      element.querySelector("#confirm-negotiation").addEventListener("click", () => {
        const terms = Object.fromEntries(fields.map(field => [field.id, element.querySelector(`[data-negotiation-field="${field.id}"]`).value]));
        closeModal();
        resolveNegotiation(terms, fields);
      });
      refresh();
    });
  }

  function resolveNegotiation(terms, fields = negotiationFields()) {
    const beforeStats = statSnapshot();
    const preferences = negotiationPreferences();
    const chosen = Object.fromEntries(fields.map(field => [field.id, field.options.find(option => option.id === terms[field.id]) || field.options[0]]));
    const score = Object.keys(preferences).reduce((sum, key) => sum + (terms[key] === preferences[key] ? 2 : ["both", "cap", "three"].includes(terms[key]) ? 1 : 0), 0);
    const cost = Object.values(chosen).reduce((sum, option) => sum + option.cost, 0);
    const outcome = state.trust < 30 || score <= 2 ? "deferred"
      : score >= 7 && cost <= 1600000 ? "stable"
        : score >= 5 ? "costly"
          : "resentful";
    const residence = terms.residence;
    const partnerIncome = terms.career === "partner_pause" || residence === "korea" ? 0
      : residence === "distance" ? Math.round((getPartner().estimatedIncome || 0) * .85)
        : getPartner().estimatedIncome || 0;
    const playerIncomeFactor = terms.career === "player_adjust"
      ? .72
      : residence === "local" ? clamp(.45 + getPlayer().portability * .06, .5, .95)
        : residence === "distance" ? .9 : 1;
    const monthlyRemittance = terms.family === "flexible" ? 700000 : terms.family === "cap" ? 300000 : 0;
    const resultByOutcome = {
      stable: {
        title: "안정적인 합의를 만들었다",
        text: "네 항목 모두에서 서로 지킬 선과 다시 의논할 날짜가 보였다. 누구도 사랑을 증명하려고 직업·가족·개인 돈을 전부 포기하지 않았다.",
        trust: 9, affection: 6, conflict: -3, stress: -2, days: 4, cost,
        reaction: "완벽히 제 뜻대로 된 건 아니지만, 제 선택이 합의문 안에 남아 있어서 좋아요."
      },
      costly: {
        title: "비싸지만 뜨거운 타협을 택했다",
        text: "둘의 핵심 조건은 지켰지만 이동·교육·가족 지원 비용이 크게 늘었다. 지금은 안도했지만 이 약속을 유지할 현금이 다음 갈등이 된다.",
        trust: 6, affection: 7, conflict: 1, stress: 5, days: 5, cost,
        reaction: "이 조건이라면 해 보고 싶어요. 대신 돈이 부족해질 때 약속을 몰래 바꾸지는 말아요."
      },
      resentful: {
        title: "원한이 남은 절충으로 끝났다",
        text: "결혼을 멈추지는 않았지만 한쪽의 중요한 조건이 비용표 아래로 밀렸다. 합의문은 생겼고, 지켜보겠다는 침묵도 함께 남았다.",
        trust: -2, affection: 1, conflict: 11, stress: 6, days: 3, cost,
        reaction: "동의는 할게요. 하지만 제가 포기한 걸 나중에 당연했다고 말하지는 마세요.",
        bad: true
      },
      deferred: {
        title: "결혼을 유예했다",
        text: "네 항목을 한 문장으로 묶을 수 없었다. 호감은 남았지만 지금 혼인하면 한 사람의 거주·경력·돈이 자동으로 사라질 가능성이 컸다.",
        trust: -3, affection: -2, conflict: 4, days: 2, cost: Math.min(200000, cost),
        reaction: "좋아하는 마음과 지금 결혼할 수 있다는 대답은 같지 않아요. 조건을 다시 만들 때까지 기다려요.",
        bad: true
      }
    };
    const result = resultByOutcome[outcome];
    result.costLabel = "결혼조건 상담·준비";
    state.negotiation = { outcome, terms: { ...terms }, score, cost: result.cost };
    state.choicesMade = [...(state.choicesMade || []), { id: "negotiation_terms", scene: scenes[state.scene]?.id, day: state.elapsedDays, success: outcome !== "deferred" }].slice(-80);
    if (outcome === "deferred") {
      state.homeCountry = null;
      state.flags.residenceAgreed = false;
    } else {
      Object.assign(state, {
        homeCountry: residence,
        monthlyLiving: residence === "korea" ? 2700000 : 1900000,
        partnerIncome,
        playerIncomeFactor,
        monthlyRemittance
      });
      state.flags.residenceAgreed = true;
      state.flags.threeAccounts = terms.money === "three";
      state.flags.playerControlsMoney = terms.money === "player_control";
    }
    applyDelta(result);
    if (state.ending) return;
    if (outcome === "stable") awardScore(180, "안정적인 결혼조건 합의", result, "play");
    else if (outcome === "costly") awardScore(110, "핵심 조건을 지킨 타협", result, "play");
    result.statChanges = statChangesSince(beforeStats);
    state.lastChoiceTitle = "거주·경력·가족지원·돈관리 조건을 한꺼번에 제안했다";
    return showResult(result);
  }

  function applyAutomaticFailure(result, item) {
    const miss = {
      empathy: "위로하려던 말이 상대의 마음을 이미 안다고 단정하는 표현처럼 들렸다.",
      charm: "가볍게 긴장을 풀려던 표현이 중요한 대답을 피하는 농담처럼 들렸다.",
      reason: "순서와 숫자를 설명했지만 왜 그 선택이 두 사람 모두에게 필요한지는 전달되지 않았다.",
      courage: "분명하게 말하려던 태도가 상대의 답을 기다리지 않는 압박처럼 들렸다."
    }[item?.check?.stat] || "하려던 말의 온도와 의도가 충분히 전달되지 않았다.";
    const behaviorMiss = {
      warm_cautious: "그녀는 바로 반박하지 않았다. 잠깐 생각하겠다는 말 뒤로 답장 간격만 길어졌다.",
      playful_social: "그녀는 웃는 이모티콘 하나를 남겼지만 평소 이어지던 농담은 거기서 끊겼다.",
      practical_planner: "그녀는 기분을 설명하는 대신 언제, 누가, 얼마를 책임지는지 다시 물었다.",
      quiet_observer: "그녀는 그 문장을 오래 바라본 뒤 필요한 말만 짧게 답했다.",
      family_centered: "그녀는 둘만의 기분보다 이 선택이 가족과 생활에 남길 일을 먼저 걱정했다.",
      ambitious_independent: "그녀는 내 제안에 자신의 선택권이 어디에 남아 있는지 되물었다.",
      passionate_impulsive: "표정이 먼저 굳었고 서운하다는 말이 빠르게 쏟아진 뒤 대화가 끊겼다.",
      guarded_survivor: "그녀는 설명을 더 보태지 않고 다음 행동을 보겠다며 한발 물러섰다."
    }[getPartner().behavior?.id] || `${getPartner().name}은 대답을 미루고 표정을 살폈다.`;
    result.title = `${scenes[state.scene]?.title || "대화"}에서 호흡이 어긋났다`;
    result.text = `‘${item?.title || "이 제안"}’을 꺼냈지만 ${miss} ${behaviorMiss}`;
    result.trust = Math.min(Number(result.trust || 0), -2);
    result.affection = Math.min(Number(result.affection || 0), -1);
    result.conflict = Math.max(Number(result.conflict || 0), 2);
    result.bad = true;
  ...
  function completeChoice(id, checkSuccess) {
    ...
    if (id === "open_contradiction_board") return openContradictionBoard();
    if (id === "open_negotiation") return openNegotiationBoard();
    ...
    if (id.startsWith("pass_")) {
      ...
      const result = { title: "이번 설명을 믿었다", text: "자료를 내밀지 않고 이번 설명을 받아들였다. 관계의 긴장은 조금 풀렸지만, 말이 사실인지 확인할 자료가 새로 생긴 것은 아니다.", reaction: partnerLine("pleased"), noApply: true, statChanges: statChangesSince(beforeStats) };
      awardScore(15, "믿음을 선택함", result, "play");
      return showResult(result);
    }
    ...
    const beforeStats = statSnapshot();
    const choiceMeta = state.pendingChoiceMeta?.id === id ? state.pendingChoiceMeta : null;
    let result = resolveAction(id, checkSuccess);
    if (!result) return;
    if (!checkSuccess && choiceMeta?.check?.auto) result = applyAutomaticFailure(result, choiceMeta);
    ...
    if (privateInterpreter) {
      result.affection = Number(result.affection || 0) - 2;
      result.intimacy = Number(result.intimacy || 0) - 4;
      result.partnerFatigue = Number(result.partnerFatigue || 0) + 2;
    }
    ...
    if (state.pendingInterpreterUsed) {
      result.text += privateInterpreter
        ? "\n\n전문 통역 덕분에 뜻은 정확했지만, 둘만의 미묘한 호흡이 끊겨 친밀감과 자연스러움은 조금 떨어졌다. 비용은 선택 순간 가계에서 빠졌다."
        : "\n\n전문 통역이 이 선택에 동행해 복잡한 뜻을 정확히 옮겼고, 비용은 선택 순간 가계에서 빠졌다.";
    }
  ...
  function resolveAction(id, checkSuccess = true) {
    ...
    const map = {
      ...
      arrival_direct: { title: "말보다 표정이 먼저 익숙해졌다", text: `${partner.name}이 메뉴판의 사진을 가리키며 웃었다. 완벽히 통하지 않아도 서로 다시 묻는 방식이 생겼다.`, trust: 8, days: 3, cost: tripQuote("standard"), costLabel: `${getCountry().name} 왕복 항공·4박 숙소·기본 데이트`, flags: { metInPerson: true } },
      arrival_interpreter: { title: "중요한 문장을 정확히 남겼다", text: "독립 통역사는 모르는 표현을 아는 척하지 않았다. 서로의 결혼 동기와 금전 기대를 각자 확인했다.", trust: 6, days: 3, cost: tripQuote("interpreter"), costLabel: `${getCountry().name} 왕복 항공·4박 숙소·독립 통역`, flags: { metInPerson: true }, evidence: { id: "arrival_record", title: "첫 만남 독립 통역 기록", type: "fact", text: "결혼 동기·직업·가족지원에 대한 양쪽 답변을 역번역해 확인했다.", source: "독립 통역", quality: 3 } },
      arrival_luxury: { title: "화려한 첫날이 남았다", text: `분위기는 분명 좋아졌다. 그러나 ${partner.name}은 내 평소 생활도 늘 이 정도인지 물었다.`, trust: 9, days: 2, cost: tripQuote("luxury"), costLabel: `${getCountry().name} 왕복 항공·고급 숙소·선물`, flags: { luxuryExpectation: true, metInPerson: true } },
      ...
      trust_no_docs: { title: "오늘의 온도는 지켰다", text: `${reactions.warm}\n다만 확인하지 않은 항목은 믿는 사실이 아니라 아직 모르는 사실로 상대 프로필에 남았다.`, trust: 7, days: 2, flag: "skippedDocs" },
      ...
      shadow_name_it: checkSuccess ? { title: "숨기던 마음에 이름이 생겼다", text: "좋아 보이는 욕구만 고르지 않았다. 질투·주도권·구원받고 싶은 마음 중 하나를 먼저 인정하고, 들어주는 것과 복종하는 것은 다르다고 합의했다.", trust: 9, affection: 8, conflict: -3, intimacy: 8, days: 2, flags: { shadowTalked: true }, reaction: `“그 마음이 있다는 것보다, 저한테 선택권을 남겨 둔 말이 더 중요해요.”` } : { title: "고백이 허락 요구처럼 들렸다", text: "욕구를 솔직히 말했지만 ‘이해한다면 해 줘야 한다’는 압박이 섞였다. 그녀는 듣는 것과 동의하는 것은 다르다고 선을 그었다.", trust: -3, affection: -2, conflict: 5, days: 1, bad: true, reaction: `“말해 준 건 고마워요. 하지만 제가 들어줘야 한다는 뜻은 아니죠.”` },
      shadow_pact: checkSuccess ? { title: "욕구보다 멈춤 규칙이 먼저 섰다", text: "누가 주도하든 중간에 마음이 바뀔 수 있고, 거절 뒤에 보복하지 않으며, 다음 날 불편했던 점을 다시 말하기로 했다. 강한 끌림이 안전한 실험이 됐다.", trust: 8, affection: 10, conflict: -4, intimacy: 12, days: 2, flags: { shadowPact: true }, reaction: `“이렇게 멈출 수 있다는 걸 아니까 오히려 더 맡길 수 있어요.”` } : { title: "역할보다 불안이 먼저 올라왔다", text: "둘 중 한 사람이 준비되지 않았다는 신호를 보냈다. 그 자리에서 멈췄고, 무엇이 불편했는지는 다음 대화에서 다시 묻기로 했다.", trust: 3, affection: 1, days: 1, reaction: `“오늘은 여기까지만 해요. 멈춘 걸 나중에 벌로 만들지 말아 줘요.”` },
      shadow_test: { title: "사랑을 확인하려다 불안을 주입했다", text: "일부러 만든 침묵과 다른 사람 이야기에 그녀의 반응은 커졌다. 질투는 확인했지만 그것이 사랑인지 버려질 두려움인지는 더 흐려졌다.", trust: -7, affection: -3, conflict: 9, stress: 4, partnerFatigue: 6, days: 2, bad: true, reaction: `“제 반응을 보고 싶어서 일부러 그랬다는 말이 더 무서워요.”` },
      shadow_bury: { title: "좋은 사람의 가면은 지켰다", text: "대화는 웃으며 끝났지만 둘 다 자기 욕구를 말하지 않았다. 이름 없는 마음은 사라지지 않고 다음 질투와 돈 문제 뒤로 숨어들었다.", trust: 1, affection: 1, shadowRisk: 6, days: 1, reaction: `“없다고 하기엔… 우리 둘 다 너무 빨리 웃은 것 같아요.”` },
      ...
      final_reconcile: { title: "발견 기록에 두 사람의 수정 표시가 남았다", text: `${partner.name}도 숨겼던 두려움 하나를 인정했고, 나도 소문을 사실처럼 읽었던 부분을 고쳤다.`, trust: state.confirmed ? 9 : 6, calm: 2, days: 2, flag: "reconciled" },
      ...
      wedding_register: checkSuccess ? { title: "식을 미루고 생활을 먼저 시작하기로 했다", text: "양가에 1년 뒤 다시 예식 예산을 공개하겠다고 같은 날짜로 약속했다. 화려한 사진은 없지만 집과 비상금에 쓸 돈을 남겼다.", trust: 7, affection: 3, conflict: 1, days: 7, cost: 800000, costLabel: "혼인신고·가족 식사", set: { weddingPlace: "혼인신고·예식 연기" }, reaction: `“미루는 게 숨기는 일이 되지만 않으면 괜찮아요. 날짜는 같이 기억해요.”` } : { title: "예식 연기가 결혼 자체를 미루는 말처럼 들렸다", text: "돈 이야기는 맞았지만 가족에게 설명할 날짜와 방법이 없었다. 두 사람은 큰 계약을 멈추고 작은 현지 가족식부터 다시 견적 내기로 했다.", trust: -2, affection: -2, conflict: 4, days: 3, set: { weddingPlace: "작은 가족식 재협의" }, bad: true },
      ...
      wedding_defer_party: { title: "잔치 비용을 집과 비상금으로 남겼다", text: "사진과 양가 식사만 남기고 큰 잔치는 미뤘다. 가족의 아쉬움은 있었지만 다음 달 생활비를 급전으로 막지 않아도 된다는 안도감이 더 컸다.", trust: 5, affection: 2, conflict: 1, stress: -2, days: 7, cost: 300000, costLabel: "사진·가족 식사", set: { weddingStyle: "잔치 연기" } },
      ...
      move_korea: checkSuccess ? { title: "한국에서 시작하는 조건에 합의했다", text: `${partner.name}의 언어 수업, 개인 통장, 일자리 준비와 가족 방문 예산을 결혼 전 합의문에 적었다. 실제 이주는 결혼식 뒤에 진행한다. 한국 경력이 없어 처음 몇 달은 배우자 수입을 0원으로 잡았다.`, trust: 7, affection: 6, days: 4, cost: 300000, costLabel: "거주 상담·번역", set: { homeCountry: "korea", monthlyLiving: 2700000, partnerIncome: 0 }, flags: { residenceAgreed: true }, reaction: `“한국에 가는 건 동의해요. 대신 제 돈과 공부 시간은 제가 정할 수 있게 해 줘요.”` } : { title: "한국행 조건에 아직 합의하지 못했다", text: "언어와 체류가 모두 내 손에 달리는 구조가 두렵다는 답이 돌아왔다. 결혼을 결정하기 전에 두 나라를 오가는 6개월 계획으로 수정했다.", trust: -2, conflict: 5, days: 4, cost: 300000, costLabel: "거주 상담·번역", set: { homeCountry: "distance", monthlyLiving: 1900000, partnerIncome: Math.round(partner.estimatedIncome * .85) }, flags: { residenceAgreed: true }, bad: true, reaction: `“당신을 싫어해서가 아니라, 제가 아무것도 못 하는 사람이 되는 게 무서워요.”` },
      move_local: checkSuccess ? { title: `${getCountry().name}에서 1년 살아 보는 조건에 합의했다`, text: `내 직업의 이동 가능성과 예상 수입 감소를 숫자로 적었다. 결혼식 뒤 실제 이주를 시작하고, 1년 뒤 다시 거주지를 결정한다.`, trust: 9, affection: 7, calm: 1, days: 5, cost: 500000, costLabel: "현지 체류·직업 상담", set: { homeCountry: "local", monthlyLiving: 1900000, playerIncomeFactor: clamp(.45 + player.portability * .06, .5, .95) }, flags: { residenceAgreed: true }, reaction: `“제 생활을 직접 보려는 선택이 고마워요. 당신 일도 같이 지킬 방법을 찾아요.”` } : { title: "현지 1년은 두 사람 모두 감당하기 어려웠다", text: "수입 감소와 체류 조건을 계산한 뒤 한국 정착 준비와 정기 현지 방문을 묶은 대안에 합의했다.", trust: 2, days: 4, cost: 500000, costLabel: "현지 체류·직업 상담", set: { homeCountry: "korea", monthlyLiving: 2600000, partnerIncome: 0 }, flags: { residenceAgreed: true }, reaction: `“마음만으로 1년을 버티긴 어려워요. 현실적인 방법으로 다시 정해요.”` },
      move_distance: checkSuccess ? { title: "6개월의 장거리 규칙에 합의했다", text: "각자의 일을 정리할 기간, 왕복 횟수, 통화 시간과 6개월 뒤 최종 거주 결정일을 적었다. 기본 생활비와 두 집·항공비를 이중 계산하지 않도록 장거리 주거안에 이동비를 묶었다.", trust: 3, affection: -1, conflict: 4, stress: 3, days: 3, cost: 300000, costLabel: "장거리 일정·서류 준비", set: { homeCountry: "distance", monthlyLiving: 1900000, playerIncomeFactor: .9, partnerIncome: Math.round(partner.estimatedIncome * .85) }, flags: { residenceAgreed: true }, reaction: `“6개월 뒤에는 꼭 다시 정해요. 끝없이 기다리는 결혼은 싫어요.”` } : { title: "장거리 제안이 결정을 미루는 말로 들렸다", text: "그녀는 결혼 뒤에도 확실한 집이 없다는 데 동의하지 않았다. 직업과 수입을 다시 비교한 뒤 한국 정착안으로 합의했다.", trust: -5, affection: -4, conflict: 8, days: 3, cost: 300000, costLabel: "장거리 일정·서류 준비", set: { homeCountry: "korea", monthlyLiving: 2700000, partnerIncome: 0 }, flags: { residenceAgreed: true }, bad: true, reaction: `“결혼했는데 언제 함께 살지조차 모르는 건 싫어요.”` },
      ...
      work_training: { title: "그녀의 경력을 다시 잇기 시작했다", text: "교육 기간에는 수입이 없었지만 스스로 하루를 꾸리는 힘이 돌아왔다. 6개월 뒤 경력과 가까운 일에서 월 240만원을 벌기 시작했다.", trust: 8, affection: 9, conflict: -4, calm: 1, partnerFatigue: -8, days: 180, cost: 2000000, costLabel: "언어·자격 교육", set: { partnerIncome: 2400000 } },
      work_immediate: { title: "빠르게 수입이 생겼다", text: "한 달 뒤 월 280만원의 수입이 생겼다. 대신 교대근무와 늦은 귀가가 늘었고, 설명하지 않은 밤은 의심의 재료가 되기 쉬워졌다.", trust: 3, affection: 3, stress: 7, partnerFatigue: 7, days: 30, set: { partnerIncome: 2800000 }, flag: "shiftWork" },
      work_forbid: { title: "집은 조용해졌고 그녀도 조용해졌다", text: `외부 일은 멈췄지만 ${partner.name}은 생활비를 쓸 때마다 내 눈치를 보기 시작했다. 구직 사이트 기록을 지우는 모습도 보였다.`, trust: -12, affection: -14, conflict: 18, intimacy: -8, partnerFatigue: 12, days: 30, set: { partnerIncome: 0 }, flag: "forbadeWork", bad: true },
      ...
      night_shift: checkSuccess ? { title: "내가 맡은 밤에는 먼저 일어났다", text: "수면은 부족했지만 약속이 실제 행동이 되자 원망은 줄었다.", trust: 9, affection: 12, conflict: -13, stress: 14, partnerFatigue: -14, days: 30, flag: "sharedNightCare" } : { title: "며칠 만에 약속이 무너졌다", text: "알람을 끄고 다시 자는 밤이 반복됐다. 말만 번듯했다는 서운함이 더 커졌다.", trust: -8, affection: -10, conflict: 15, stress: 10, partnerFatigue: 14, days: 30, bad: true },
      night_helper: { title: "두 사람이 다시 잠을 잤다", text: "한 달의 도움으로 몸과 판단력이 회복됐다. 돈은 줄었지만 큰 싸움 하나를 피했다.", trust: 5, affection: 7, conflict: -12, calm: 3, partnerFatigue: -18, days: 30, cost: 2500000, costLabel: "산후·야간 돌봄 도움" },
      ...
      nochild_debt: { title: "생활을 줄이고 빚을 갚았다", text: "큰 지출을 멈추고 현금 중 생활 비상금 500만원을 제외한 금액을 급전 상환에 넣었다. 이후에도 원금이 남으면 이자는 계속 붙는다.", trust: 5, affection: 2, conflict: -6, calm: 2, days: 180, debtPayment: Math.max(0, state.cash - 5000000), debtLabel: "가계 긴축 상환", set: { monthlyLiving: Math.max(1900000, state.monthlyLiving - 400000) } },
      ...
      care_daycare: { title: "맞벌이와 보육을 함께 시작했다", text: "두 사람의 수입은 유지됐지만 등원 시간과 아이의 적응 문제를 함께 감당해야 했다.", trust: 4, affection: 3, stress: 8, partnerFatigue: 4, days: 90, cost: 800000, costLabel: "보육 초기비", set: { monthlyLiving: state.monthlyLiving + 350000, partnerIncome: Math.max(state.partnerIncome, 2400000) } },
      care_partner_home: { title: "한 사람의 경력으로 돌봄 비용을 냈다", text: `${partner.name}은 아이를 돌봤지만 수입과 외부 관계가 사라지며 고립감을 느꼈다.`, trust: 2, affection: 2, conflict: 8, stress: 9, partnerFatigue: 12, days: 90, set: { partnerIncome: 0 }, flag: "partnerHomeCare" },
      care_player_reduce: { title: "내 수입을 줄이고 시간을 냈다", text: "현금은 줄었지만 돌봄이 한 사람의 일이 되지 않았다. 아이의 하루를 두 사람 모두 알게 됐다.", trust: 9, affection: 11, conflict: -10, stress: 5, partnerFatigue: -12, days: 90, set: { playerIncomeFactor: Math.max(.55, (state.playerIncomeFactor || 1) - .2) } },
  ...
  function evaluateEvidence(index, evidenceId, statement) {
    ...
    if (correct) {
      ...
      gainCertainty(10);
      passTime(1);
      const result = { title: "모순을 정확히 짚었다", text: `${withJosa(evidence.title, "을", "를")} 제시했다.\n\n${statement.success}`, days: 1, noApply: true, statChanges: statChangesSince(beforeStats) };
      awardScore(180, "증거 연결 성공", result, "evidence");
      showResult(result);
    } else {
      state.mistakes += 1;
      ...
      state.calm = clamp(state.calm - 1, 0, 9);
      passTime(1);
      showResult({ title: "이 증거로는 진술을 반박할 수 없었다", text: `${evidence.title}은(는) 방금 한 말을 직접 틀렸다고 증명하지 못한다. ${evidence.type === "rumor" ? "출처가 약한 소문을 사실처럼 꺼내 신뢰를 크게 잃었다." : "관련은 있어 보여도 다른 설명이 가능했다."}`, bad: true, days: 1, noApply: true, statChanges: statChangesSince(beforeStats) });
    }
  }
  ...
  function showResult(result) {
    ...
    const unlockedLabels = [
      ...
      ...(result.shadowUnlocked || []).map(id => `어두운 욕구 · ${shadowData(id).label}`)
    ];
    const unlocked = unlockedLabels.length
      ? `<button class="profile-unlock-panel" data-open-partner type="button"><span>새로 알게 된 정보</span><strong>${unlockedLabels.map(escapeHtml).join(" · ")}</strong><small>눌러서 상대 프로필에서 확인</small></button>`
      : "";
  ...
  function playResultEffects(result) {
    const events = [];
    if (result.profileUnlocked?.length) {
      const labels = result.profileUnlocked.map(key => PROFILE_LABELS[key] || key);
      events.push({ type: "profile", kicker: "PROFILE UPDATED", title: labels.length === 1 ? labels[0] : `새 정보 ${labels.length}개`, detail: labels.map(label => `· ${label}`).join(" ") });
    }
    if (result.secretUnlocked) {
      const secret = getPartner().privateTraits?.secrets?.find(item => item.id === result.secretUnlocked);
      if (secret) events.push({ type: "profile", kicker: "HIDDEN FACT", title: secret.label, detail: secret.detail });
    }
    (result.shadowUnlocked || []).forEach(id => events.push({ type: "shadow", kicker: "SHADOW REVEALED", title: shadowData(id).label, detail: shadowData(id).impulse }));
    const featuredScores = (result.scoreEvents || []).filter(item => ["need", "partner-need", "shadow"].includes(item.type));
    if (featuredScores.length) {
      const amount = featuredScores.reduce((sum, item) => sum + item.amount, 0);
      events.push({ type: featuredScores.some(item => item.type === "shadow") ? "shadow" : "need", kicker: featuredScores.some(item => item.type === "shadow") ? "SHADOW RESONANCE" : "NEED CLEAR", title: featuredScores.map(item => item.label).join(" · "), detail: `+${amount.toLocaleString("ko-KR")}점 · 현재 총점 ${(state.score || 0).toLocaleString("ko-KR")}점` });
    }
    if (!events.length) return;
    if (effectTimer) clearTimeout(effectTimer);
    const overlay = $("#effect-overlay");
    const card = $("#effect-card");
    const queue = events.slice(0, 2);
    let index = 0;
    const next = () => {
      ...
      card.className = `effect-card is-${event.type}`;
      $("#effect-kicker").textContent = event.kicker;
      $("#effect-title").textContent = event.title;
      $("#effect-detail").textContent = event.detail;
      overlay.hidden = false;
      card.style.animation = "none";
      void card.offsetWidth;
      card.style.animation = "";
      effectTimer = setTimeout(next, window.__KG_FAST_TEST__ ? 0 : 760);
    };
    next();
  }
  ...
  function resolveDecision(id, checkSuccess = true) {
    ...
    if (id === "decide_marry") {
      ...
      state.partnerIncome = state.homeCountry === "korea" ? 0 : state.homeCountry === "distance" ? Math.round(partnerBaseIncome * .65) : partnerBaseIncome;
      state.monthlyCrossBorder = state.homeCountry === "distance" ? 0 : state.homeCountry === "local" ? 180000 : 250000;
  ...
  function resolveFamilyDecision(id) {
    ...
    if (id === "family_stay") {
      ...
      if ((state.shadowRisk || 0) >= 70) return finishGame("shadow_spiral");
  ...
  function finishGame(type) {
    ...
    const endings = {
      ...
      shadow_spiral: ["BAD END · 욕구가 관계를 삼켰다", "끌림은 강했지만 선택권은 점점 사라졌다", "질투와 통제, 구원받고 싶은 마음을 합의 없이 반복했다. 점수와 호감이 남아 있어도 한쪽이 거절할 수 없는 관계는 함께 살 수 있는 안전한 관계가 되지 못했다.", "nightlife-secret"]
    };
    renderEnding(endings[type], type);
  }
  ...
  function renderEnding(data, type) {
    ...
    const retry = $("#retry-case");
    const canRematch = REMATCH_ENDINGS.has(type) && !state.married;
    retry.dataset.mode = canRematch ? "rematch" : "retry";
    retry.textContent = canRematch ? "손실을 안고 새 상대 3명 만나기" : "다른 가능성으로 다시";
    showScreen("ending");
  }
  ...
  function openPlayerProfile() {
    ...
    const growth = state.growthLog.length ? state.growthLog.slice(0, 6).map(item => `<li><b>${escapeHtml(statLabel(item.stat))} +${item.amount}</b><span>${escapeHtml(item.reason)} · ${item.day}일째</span></li>`).join("") : "<li><span>아직 능력치가 오른 기록은 없습니다. 같은 능력을 잘 활용한 반응 3회 또는 자기계발로 성장합니다.</span></li>";
    const needs = (player.needs || []).map(need => {
      const cleared = (state.fulfilledNeeds || []).includes(need.id);
      return `<article class="need-row${cleared ? " is-cleared" : ""}"><strong>${escapeHtml(need.label)}</strong><span>${cleared ? "✓ 달성" : `+${needScoreValue(need).toLocaleString("ko-KR")}점`}</span><small>${cleared ? "상대에 대해 필요한 사실을 실제로 알아낸 뒤 달성됨" : "관련 정보가 열리고 조건이 맞으면 자동 확인"}</small></article>`;
    }).join("");
    const scoreHistory = (state.scoreLog || []).length
      ? state.scoreLog.slice(0, 8).map(item => `<li><span>${escapeHtml(item.label)}</span><b>+${item.amount.toLocaleString("ko-KR")}</b></li>`).join("")
  ...
  function partnerProfileValue(key) {
    ...
    const values = {
      ...
      claimedAge: `${partner.age}세라고 소개함`, claimedCity: `${partner.city}에 산다고 소개함`, claimedJob: `${withJosa(partner.job, "이라고", "라고")} 소개함`,
      ...
      personalityCore: behavior.name || partner.personality, mbti: partner.mbti || behavior.mbti || "미확인", speechTone: behavior.tone, motive: partner.motive, intimacyBoundary: behavior.intimacyBoundary,
      childIntent: childText,
      desiredChildren: `${Number.isFinite(traits.desiredChildren) ? traits.desiredChildren : "?"}명 정도를 상상하고 있음`,
      photoReality: partner.profileDifference,
      attraction: traits.attractiveness >= 8 ? "내 눈에는 매우 매력적으로 느껴짐" : traits.attractiveness >= 6 ? "호감이 가는 인상" : "사진보다 대화와 표정에서 매력을 더 느끼는 편",
      height: `${traits.height}cm`, bodyType: traits.bodyType,
  ...
  function openPartnerProfile() {
    state.profileUnread = 0;
    updateSidebar();
    const partner = getPartner();
    const traits = partner.privateTraits || {};
    const behaviorInfo = partner.behavior || {};
    ...
    const behaviorFields = ["mbti", "personalityCore", "speechTone", "responseRhythm", "affectionLanguage", "conflictStyle", "moneyStyle", "independence", "jealousy", "motive"];
    const bodyFields = ["photoReality", "attraction", "height", "bodyType", "weight", "braSize", "measurements", "tattoo", "health", "chemistry", "intimacyBoundary", "childIntent", "desiredChildren"];
    const knownFields = keys => keys.filter(key => state.knownProfile?.[key]).map(key => dossierField(key, "")).join("");
    const lockedMysteries = (keys, text) => {
      const count = keys.filter(key => !state.knownProfile?.[key]).length;
      if (!count) return "";
      const visible = Math.min(3, count);
      const slots = Array.from({ length: visible }, (_, index) => `<div class="dossier-field is-locked mystery-slot"><dt>미확인 정보 ${index + 1}</dt><dd><span>████████</span><small>${escapeHtml(text)}</small></dd></div>`).join("");
      return `${slots}${count > visible ? `<div class="dossier-lock-summary"><strong>암흑 속 정보 ${count - visible}개 더 있음</strong><span>무엇이 숨겨졌는지조차 아직 알 수 없습니다.</span></div>` : ""}`;
    };
    const initial = knownFields(initialFields) + lockedMysteries(initialFields, "직접 묻거나 원본을 확인해야 열립니다.");
    const behavior = knownFields(behaviorFields) + lockedMysteries(behaviorFields, "여러 상황에서 반복되는 행동을 봐야 열립니다.");
    const body = knownFields(bodyFields) + lockedMysteries(bodyFields, "실제 만남·상호 동의·건강 대화 중 알맞은 문이 필요합니다.");
    ...
    const knownCount = Object.keys(state.knownProfile || {}).length + state.revealedSecrets.length + (state.knownPartnerShadows || []).length;
    ...
    const evidenceRows = state.evidence.length ? state.evidence.map(item => evidenceCardHtml(item, false)).join("") : `<p class="preview-empty">아직 확인한 자료가 없습니다.</p>`;
    const partnerNeeds = (behaviorInfo.relationshipNeeds || []).map((need, index) => {
      const id = `${behaviorInfo.id}_${index}`;
      const cleared = (state.fulfilledPartnerNeeds || []).includes(id);
      return `<article class="need-row${cleared ? " is-cleared" : ""}"><strong>${escapeHtml(need)}</strong><span>${cleared ? "✓ 내가 충족" : "관찰 중"}</span><small>${cleared ? "내 반복 행동이 그녀의 관계 니즈에 실제로 닿았습니다." : "한 번의 정답보다 같은 태도가 반복되는지를 봅니다."}</small></article>`;
    }).join("");
    const needsPanel = state.knownProfile?.personalityCore
      ? `<div class="needs-panel">${partnerNeeds}</div><article class="taboo-card"><span>금기</span><strong>${escapeHtml(behaviorInfo.taboo || "아직 뚜렷한 금기를 알지 못함")}</strong></article>`
      : `<div class="dossier-lock-summary"><strong>관계 니즈와 금기는 아직 암흑</strong><span>좋아하는 행동과 부담스러워하는 행동이 반복되어야 열립니다.</span></div>`;
    const knownShadows = state.knownPartnerShadows || [];
    const partnerShadows = traits.shadowDesires || [];
    const shadowRows = partnerShadows.map(item => {
      const known = knownShadows.includes(item.id);
      if (!known) return `<article class="shadow-card is-locked"><header><span>◼</span><strong>████████</strong><b>미확인</b></header><p>질투·돈·주도권·의존이 걸린 선택에서 행동을 반복해서 봐야 합니다.</p></article>`;
      const data = shadowData(item.id);
      const intensity = "●".repeat(item.intensity || 2) + "○".repeat(4 - (item.intensity || 2));
      return `<article class="shadow-card"><header><span>${data.icon}</span><strong>${escapeHtml(data.label)}</strong><b>${intensity}</b></header><p>${escapeHtml(data.impulse)}</p><dl><div><dt>안전하게 표현할 때</dt><dd>${escapeHtml(data.regulated)}</dd></div><div><dt>왜곡될 때</dt><dd>${escapeHtml(data.distorted)}</dd></div></dl></article>`;
    }).join("");
    const contradiction = knownShadows.length >= 2
      ? `<article class="contradiction-card"><span>숨은 모순</span><strong>${escapeHtml(behaviorInfo.contradiction || "말과 욕구 사이의 모순을 함께 확인했습니다.")}</strong></article>`
      : `<article class="contradiction-card is-locked"><span>숨은 모순</span><strong>████████████</strong><small>어두운 욕구 두 개를 모두 알아야 문장이 열립니다.</small></article>`;
    const dossier = `<div class="partner-dossier"><header class="dossier-hero"><button class="dossier-photo" data-photo-zoom type="button" title="사진 확대" style="${photoStyle(partner.art, "woman", currentPhotoMode())}"><span>🔍 확대</span></button><div><span>${escapeHtml(getRoute().name)}에서 만남</span><h3>${escapeHtml(partner.name)}</h3><p>호감 ${state.affection} · 그녀의 신뢰 ${state.trust} · 상대 피로 ${state.partnerFatigue} · 사실 확인도 ${state.certainty}<br>그림자 공명 ${state.shadowResonance || 0} · 위험 ${state.shadowRisk || 0}/100</p></div></header><nav class="dossier-tabs" aria-label="상대 프로필 항목"><button class="is-active" data-profile-tab="intro" type="button">기본 정보</button><button data-profile-tab="life" type="button">성격·생활</button><button data-profile-tab="private" type="button">사생활·그림자</button></nav><section class="dossier-panel" data-profile-panel="intro"><section class="dossier-section"><h3>처음 들은 소개</h3><p>실제로 듣거나 확인한 내용만 밝게 표시됩니다.</p><dl class="dossier-grid">${initial}</dl></section></section><section class="dossier-panel" data-profile-panel="life" hidden><section class="dossier-section"><h3>행동으로 드러난 성격</h3><p>한 번의 정답보다 반복되는 말투와 선택을 기록합니다.</p><dl class="dossier-grid">${behavior}</dl></section><section class="dossier-section"><h3>그녀가 관계에서 원하는 것</h3><p>상대도 나를 평가합니다. 충족한 항목은 전체 점수와 그녀의 니즈 점수에 함께 반영됩니다.</p>${needsPanel}</section></section><section class="dossier-panel" data-profile-panel="private" hidden><section class="dossier-section is-shadow"><h3>그녀의 어두운 욕구</h3><p>욕구는 범죄 증거가 아닙니다. 합의된 공명과 상대 선택권을 지우는 왜곡을 구분합니다.</p><div class="shadow-grid">${shadowRows}</div>${contradiction}</section><section class="dossier-section is-private"><h3>서로 동의해 알게 된 사생활</h3><p>${escapeHtml(bodyNote)}</p><dl class="dossier-grid">${body}</dl></section><section class="dossier-section"><h3>아직 말하지 않은 사정</h3><p>관련 사건·자발적 고백·원본 확인 중 맞는 조건이 있어야 이름이 드러납니다.</p><div class="secret-profile-list">${secretRows}</div></section><details class="dossier-section dossier-evidence"><summary>발견 기록과 확인 자료 ${state.evidence.length}개</summary><div class="evidence-list">${evidenceRows}</div></details></section></div>`;
    openModal("상대 프로필", `${partner.name} · ${knownCount}개 발견`, dossier, element => {
      const buttons = [...element.querySelectorAll("[data-profile-tab]")];
      const panels = [...element.querySelectorAll("[data-profile-panel]")];
      buttons.forEach(button => button.addEventListener("click", () => {
        buttons.forEach(item => item.classList.toggle("is-active", item === button));
        panels.forEach(panel => { panel.hidden = panel.dataset.profilePanel !== button.dataset.profileTab; });
      }));
      element.querySelector("[data-photo-zoom]")?.addEventListener("click", () => {
        const mode = currentPhotoMode();
        openModal("사진 확대", `${partner.name} · ${mode === "profile" ? "프로필 원본" : mode === "real" ? "실제 만남" : "결혼 생활"}`, `<div class="photo-zoom"><div class="photo-zoom-image" style="${photoStyle(partner.art, "woman", mode)}"></div><p>${mode === "profile" ? "아직 실제로 만나기 전입니다. 확대해도 프로필의 필터 사진만 보입니다." : escapeHtml(partner.profileDifference || "실제 만남에서 확인한 모습입니다.")}</p></div>`);
      });
    });
  }
  ...
  function renderMoneyModal() {
    ...
    const totalHousing = normalized.filter(item => item.direction === "housing").reduce((sum, item) => sum + item.amount, 0);
    ...
    const childCost = state.children * 785000;
    const pregnancyCost = state.pregnancy ? 320000 : 0;
    const sharedPersonalCost = state.married ? (state.monthlySharedPersonal || SHARED_PERSONAL_MONTHLY) : 0;
    const crossBorderCost = state.married ? (state.monthlyCrossBorder || CROSS_BORDER_MONTHLY) : 0;
    const settlingCost = state.married && state.monthsMarried <= 6 ? EARLY_SETTLING_MONTHLY : 0;
    const recurring = state.married ? state.monthlyLiving + player.monthlyCommitment + state.monthlyRemittance + childCost + pregnancyCost + sharedPersonalCost + crossBorderCost + settlingCost + (state.monthlyHousing || 0) : player.singleLiving + player.monthlyCommitment;
    const monthlyIncome = playerIncome + (state.married ? state.partnerIncome || 0 : 0);
    const emergencyInterest = state.debt ? Math.max(100000, Math.round(state.debt * .025)) : 0;
    const monthlyBalance = monthlyIncome - recurring - emergencyInterest;
    const sixMonthProjection = state.cash + monthlyBalance * 6;
    ...
    const tabs = [["all", "전체"], ["income", "수입"], ["expense", "생활 지출"], ["housing", "주거"], ["repayment", "빚 상환"]];
    const homeName = housingOptionById(state.housingChoice)?.name || "아직 계약 전";
    const housingAssetValue = (state.homeValue || 0) + (state.housingDeposit || 0) + (state.housingAsset || 0);
    const netHousing = housingAssetValue - (state.mortgageBalance || 0);
    const sharedCosts = (state.monthlyRemittance || 0) + childCost + pregnancyCost + crossBorderCost + settlingCost;
    const body = `<div class="money-summary"><div><span>현재 현금</span><b>${formatWon(state.cash)}</b></div><div><span>급전</span><b>${formatWon(state.debt)}</b></div><div><span>월 예상 잔액</span><b>${formatWon(monthlyBalance)}</b></div></div><div class="money-forecast"><strong>6개월 공동생활 전망</strong><span>기준 수입이 유지되면 현금 약 ${formatWon(sixMonthProjection)}</span><small>실직·휴직·자영업 적자·프리랜서 급락은 이 전망보다 나빠질 수 있음 · ${escapeHtml(debtRisk)}</small></div><section class="housing-ledger"><div><span>현재 주거</span><strong>${escapeHtml(homeName)}</strong></div><div><span>집값·보증금·남은 종잣돈</span><strong>${formatWon(housingAssetValue)}</strong></div><div><span>주담대 잔액</span><strong>${formatWon(state.mortgageBalance || 0)}</strong></div><div><span>주거 순자산</span><strong>${formatWon(netHousing)}</strong></div></section><div class="money-breakdown"><div class="money-row is-income"><span>내 월 기준수입</span><strong>+${formatWon(playerIncome)}</strong></div><div class="money-row is-income"><span>배우자 월 기준수입</span><strong>+${formatWon(state.married ? state.partnerIncome || 0 : 0)}</strong></div><div class="money-row"><span>${state.married ? "부부 생활비·내 고정비" : "개인 생활비·고정비"}</span><strong>-${formatWon(state.married ? state.monthlyLiving + player.monthlyCommitment : player.singleLiving + player.monthlyCommitment)}</strong></div><div class="money-row"><span>두 사람 개인·교통·통신비</span><strong>-${formatWon(sharedPersonalCost)}</strong></div><div class="money-row is-housing"><span>월세 또는 주담대·관리비</span><strong>-${formatWon(state.monthlyHousing || 0)}</strong></div><div class="money-row"><span>가족 지원·양육·임신·왕래·정착</span><strong>-${formatWon(sharedCosts)}</strong></div><div class="money-row"><span>급전 월 예상 이자</span><strong>-${formatWon(emergencyInterest)}</strong></div></div><div class="money-tabs">${tabs.map(([id, label]) => `<button class="money-tab${id === moneyFilter ? " is-active" : ""}" data-money-filter="${id}" type="button">${label}</button>`).join("")}</div><div class="money-summary"><div><span>기록된 총수입</span><b>+${formatWon(totalIncome)}</b></div><div><span>생활·선택 지출</span><b>-${formatWon(totalExpense)}</b></div><div><span>주거 자기자본</span><b>-${formatWon(totalHousing)}</b></div><div><span>급전 상환</span><b>-${formatWon(totalRepayment)}</b></div></div><div class="money-breakdown">${rows}</div><p class="warning-box">주담대는 집에 담보가 있는 장기부채이고, 생활비 부족으로 생긴 급전과 별도로 계산합니다. 급전에는 월 2.5%의 게임상 이자와 스트레스가 붙고, 600만원부터 부부갈등도 매달 증가합니다.</p>`;
  ...
  function toggleInterpreter() {
    if (!state || state.ending) return;
    state.interpreterOn = !state.interpreterOn;
    renderGame();
    showToast(state.interpreterOn ? "전문 통역을 켰습니다. 다음 고위험 대화의 뜻을 더 정확히 옮기고 비용을 표시합니다." : "전문 통역을 껐습니다. 비용은 없지만 지금까지 쌓은 소통 맥락으로 직접 대화합니다.");
  }
  ...
  function beginRematch() {
    if (!state || !REMATCH_ENDINGS.has(state.ending) || state.married) return;
    const previous = state;
    const loss = previous.routeId === "broker" ? 5000000 : 3000000;
    const cash = Math.max(0, previous.cash - loss);
    const addedDebt = Math.max(0, loss - previous.cash);
    const elapsedDays = previous.elapsedDays + 21;
    const lossEntry = {
      label: `${getPartner().name} 관계 정리·취소·재매칭 손실`,
      amount: loss,
      direction: "expense",
      day: elapsedDays
    };
    rematchCarry = {
      ...
      cash,
      debt: previous.debt + addedDebt,
      spent: previous.spent + loss,
      score: previous.score || 0,
      scoreLog: [...(previous.scoreLog || [])],
      stress: clamp(previous.stress + 12, 0, 92),
      elapsedDays,
      daysLeft: Math.max(previous.routeId === "broker" ? 135 : 150, previous.daysLeft - 21),
      moneyLog: [lossEntry, ...(previous.moneyLog || [])],
      compulsionPressure: clamp((previous.compulsionPressure || 0) + 5, 0, 100)
    };
    setup.playerId = previous.playerId;
    setup.countryId = previous.countryId;
    setup.routeId = previous.routeId;
    setup.planId = previous.planId;
    setup.partnerId = null;
    setup.partnerSnapshot = null;
    setup.step = 4;
    setup.quickStart = false;
  ...
  $("#mobile-partner").addEventListener("click", openPartnerProfile);
  $("#open-partner-profile-hud").addEventListener("click", openPartnerProfile);
  $("#interpreter-toggle").addEventListener("click", toggleInterpreter);
  $("#mobile-interpreter").addEventListener("click", toggleInterpreter);
  ...
  $("#retry-case").addEventListener("click", () => {
    if ($("#retry-case").dataset.mode === "rematch") return beginRematch();
