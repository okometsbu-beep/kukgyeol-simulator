(function () {
  ...
  const SAVE_KEY = "genuine-evidence-save-v2";
  ...
  function photoStyle(art, kind = "woman", mode = "real") {
    ...
    return `background-image:url('./assets/women-${art.sheet}.webp');background-size:${cols * 100}% ${rows * 100}%;background-position:${x}% ${y}%;${polish}`;
  }
  ...
  function renderSetup() {
    ...
    items.forEach(item => {
      ...
      if (setup.step === 0) {
        ...
        button.innerHTML = `<div class="selection-photo" style="${photoStyle(item.art, "woman", "profile")}"></div><div class="selection-body"><h3>${item.name} · ${item.age}세</h3><p>${item.job}</p><span class="selection-tag">${item.profileLook}</span><span class="selection-cost">${item.boundary}</span></div>`;
      }
  ...
  function buildCandidates() {
    ...
    const weighted = [...pool].sort((a, b) => {
      const youthBias = setup.routeId === "broker" ? (a.age - b.age) * .7 : 0;
      return youthBias + (Math.random() - .5) * 18;
    });
  ...
  function startGame() {
    ...
    const partner = DATA.people.find(x => x.id === setup.partnerId);
  ...
  function phaseName() {
    if (state.scene < 4) return "첫 연락";
    if (state.scene < 11) return "현지 연애";
    if (state.scene < 20) return "진실 확인";
    if (state.scene < 23) return "결혼 준비";
    if (state.scene < 31) return "신혼 생활";
    if (state.scene < 36) return "아이 계획";
    return "가족의 삶";
  }
  ...
  function sceneContent() {
    ...
    switch (scene.id) {
      ...
      case "arrival":
        return { ...common, mood: "긴장한 웃음", context: `현지 첫 만남 · 공항 도착`, text: `입국장을 나서자 ${partner.name}이 먼저 손을 흔든다. 프로필은 ${partner.profileLook}이었고, 실제로 보니 ${partner.profileDifference}. 사진과 다르다는 놀람보다 화면 밖의 사람이 눈앞에 있다는 긴장이 더 컸다.\n\n“직접 보니까 저도 말이 안 나오네요. 우선 밥부터 먹을까요?”`, choices: [
      ...
      case "wedding_day":
        return { ...common, speaker: `${partner.name}의 가족`, mood: "갑작스러운 요구", context: `결혼식 당일 · 예식 40분 전`, text: `예식 직전, 가족 한 사람이 약속에 없던 현금 예물을 요구했다. “여기서는 원래 이렇게 한다”는 말과 “지금 거절하면 체면이 무너진다”는 말이 겹친다.\n\n${partner.name}은 난처한 얼굴로 내 쪽을 본다. 지금 대응은 돈뿐 아니라 두 사람이 같은 편인지도 시험한다.`, choices: [
      ...
      case "move_country":
        return { ...common, speaker: partner.name, mood: "현실적인 표정", context: `혼인 신고 뒤 · 첫 거주지를 정하는 밤`, text: `“결혼식은 끝났지만 이제 진짜 어려운 걸 정해야 해요. 제가 한국으로 가면 일과 가족을 두고 가야 하고, 당신이 여기 오면 당신 수입이 흔들려요. 계속 떨어져 사는 것도 쉬운 답은 아니고요.”`, choices: [
          choice("move_korea", "한국에서 신혼을 시작한다", "항공·서류·초기 정착비를 쓰고 그녀가 한국 생활을 시작한다.", "지금 250만원 · 당신 수입 유지, 그녀의 적응 부담", 2500000, "investigate"),
          choice("move_local", `${country.name}에서 1년 살아 본다`, "내 일을 줄이고 현지에 머물며 그녀의 생활권 안에서 결혼을 시험한다.", "지금 400만원 · 관계 이해↑, 내 수입 감소", 4000000, "romance"),
          choice("move_distance", "당분간 두 나라를 오간다", "각자 일을 유지하고 두 달마다 만나기로 한다.", "지금 150만원 · 경력 유지, 외로움과 이중생활 위험", 1500000, "risky")
        ] };
      case "new_home":
  ...
  function weddingPlaceChoices() {
    const country = getCountry();
    return [
      choice("wedding_local", `${country.name}에서 결혼식을 한다`, "그녀의 가족과 친구들이 오기 쉬운 현지 예식을 준비한다.", "기본 준비 600만원 · 그녀와 현지 가족 호감↑", 6000000, "romance"),
      choice("wedding_korea", "한국에서 결혼식을 한다", "내 가족과 지인이 오기 쉬운 한국 예식을 준비하고 그녀의 가족 일부를 초대한다.", "기본 준비 1,200만원 · 내 쪽은 편함, 그녀의 외로움 위험", 12000000, "investigate"),
      choice("wedding_both", "두 나라에서 한 번씩 한다", "현지 가족식과 한국 예식을 모두 열어 양가를 직접 만난다.", "기본 준비 1,800만원 · 양가 호감↑, 빚 위험 큼", 18000000, "romance")
    ];
  }
  ...
  function pregnancyResultContent(common) {
    if (state.pregnancy) return { ...common, speaker: getPartner().name, mood: "울다가 웃음", context: `임신 확인일 · 병원 앞`, bg: "newborn", portrait: false, text: `검사 결과가 확인됐다. ${getPartner().name}은 종이를 몇 번이나 다시 본 뒤 내 손을 잡았다.\n\n“기쁘면서도 무서워요. 이제부터는 말이 아니라 진짜 생활이 바뀌는 거죠?”`, choices: [
  ...
  function birthContent(common) {
    ...
    return { ...common, speaker: "나", mood: "기쁨과 두려움", context: `출산 당일 · 가족이 한 명 늘어난 날`, portrait: false, bg: "newborn", text: `긴 시간이 지나 아이의 울음이 들렸다. ${getPartner().name}은 지친 얼굴로 웃었고, 나는 기쁨과 함께 앞으로 감당할 돈과 밤들이 한꺼번에 떠올랐다.\n\n결혼식과 마찬가지로 출산도 엔딩이 아니다. 이제부터는 두 사람이 부모로서 함께 버틸 수 있는지가 시작된다.`, choices: [
  ...
  function decisionChoices() {
    const route = getRoute();
    const brokerBalance = 19320000 - route.initialCost - (state.flags.paidReservation ? 3000000 : 0);
    const choices = [
  ...
  function renderGame() {
    ...
    $("#stat-charm").textContent = getPlayer().charm;
    $("#stat-empathy").textContent = getPlayer().empathy;
    $("#stat-reason").textContent = getPlayer().reason;
    $("#stat-courage").textContent = getPlayer().courage;
    ...
    const photoMode = state.scene < 4 ? "profile" : "real";
  ...
  function statValue(stat) {
    ...
    if (stat === "calm") return state.calm;
    return Number(getPlayer()[stat] || 5);
  }
  ...
  function chanceFor(check) {
    ...
    return clamp(Math.round(check.base + main + assist + relationship + stressPenalty), 5, 95);
  }
  ...
  function runRoll(item) {
    ...
    setTimeout(() => {
      ...
      result.textContent = success ? "성공" : "실패";
  ...
  function handleChoice(itemOrId) {
  ...
  function resolveAction(id, checkSuccess = true) {
    ...
    const partnerScam = mystery.culprit === "partner" || mystery.culprit === "both";
    const hiddenReason = {
    ...
    const map = {
      ...
      wedding_local: { title: `${getCountry().name}에서 먼저 축하받기로 했다`, text: `${partner.name}은 가족이 직접 볼 수 있다는 말에 안도했다. 내 가족은 아쉬워했지만 영상과 한국 방문 일정을 따로 잡기로 했다.`, trust: 7, affection: 10, conflict: -3, days: 21, cost: 6000000, costLabel: "현지 결혼식 기본 준비", set: { weddingPlace: `${getCountry().name} 예식` } },
      wedding_korea: { title: "한국 예식을 준비하기로 했다", text: `내 가족은 편해졌지만 ${partner.name}은 자기 가족이 몇 명이나 올 수 있는지부터 물었다. 초대와 통역을 제대로 챙기지 않으면 외로운 결혼식이 될 수 있다.`, trust: 2, affection: 2, conflict: 5, days: 28, cost: 12000000, costLabel: "한국 결혼식 기본 준비", set: { weddingPlace: "한국 예식" } },
      wedding_both: { title: "두 나라의 가족을 모두 만나기로 했다", text: "두 번의 예식은 피곤하고 비쌌지만 어느 한쪽만 손님이 되는 느낌은 줄었다. 문제는 신혼을 시작하기도 전에 통장 잔액이 빠르게 줄었다는 점이다.", trust: 9, affection: 12, conflict: -4, stress: 12, days: 42, cost: 18000000, costLabel: "양국 결혼식 기본 준비", set: { weddingPlace: "양국 예식" } },
      ...
      move_korea: { title: "한국행 비행기에 함께 올랐다", text: `${partner.name}은 창밖을 오래 보다가 “이제부터 저는 모르는 사람처럼 될 수도 있어요. 그때도 제 말을 기다려 줘요”라고 말했다.`, trust: 6, affection: 7, stress: 6, days: 30, cost: 2500000, costLabel: "항공·서류·초기 정착", set: { homeCountry: "korea", monthlyLiving: 2700000, playerIncomeFactor: 1 } },
      move_local: { title: `${getCountry().name}에서 생활을 시작했다`, text: "내가 낯선 사람이 되어 보니 통역과 생활 정보가 없는 답답함을 조금 이해하게 됐다. 대신 한국의 일은 줄었고 수입도 함께 내려갔다.", trust: 9, affection: 8, calm: 1, days: 30, cost: 4000000, costLabel: "현지 이주·체류 준비", set: { homeCountry: "local", monthlyLiving: 1900000, playerIncomeFactor: .65 } },
      move_distance: { title: "두 나라를 오가는 신혼이 시작됐다", text: "각자의 일은 지켰지만 같은 집에서 쌓이는 평범한 기억은 적었다. 항공권 날짜가 사랑의 리듬이 되었고, 연락이 끊기는 날의 의심은 더 커졌다.", trust: 1, affection: 2, conflict: 7, stress: 5, days: 45, cost: 1500000, costLabel: "첫 왕복 항공·숙소", set: { homeCountry: "distance", monthlyLiving: 2300000, playerIncomeFactor: .9 } },
  ...
  function resolveDecision(id) {
    ...
    if (id === "decide_marry") {
      const cost = getRoute().id === "broker" ? Math.max(0, 19320000 - getRoute().initialCost - (state.flags.paidReservation ? 3000000 : 0)) : 2000000;
  ...
  function continueGame() {
    try {
      ...
      if (!saved || saved.version !== 2) throw new Error("invalid save");
      state = saved;
      showScreen("game");
      renderGame();
    } catch (_) {
